...
@cuda.jit
def _compute_energies_integers_kernel(coefficient, position_mask, energies_array_out):
    idx = cuda.grid(1)
...
def _compute_all_energies_integers(
    hamiltonian_list_representation: List[Tuple[Union[float, int], Tuple[int, ...]]],
    dtype=np.float32,
):
    number_of_qubits = (
        max([max(interaction[1]) for interaction in hamiltonian_list_representation])
        + 1
    )
    ...
    for coeff, qubit_indices in hamiltonian_list_representation:
        ...
        _compute_energies_integers_kernel.forall(dim)(
            coeff, position_mask, energies_array
        )
...
def get_all_two_1s_bitstrings_cuda(
    number_of_qubits: int, dtype=np.int32, include_one_1s_bitstrings: bool = False
):
    number_of_pairs = number_of_qubits * (number_of_qubits - 1) // 2
    bitstrings_array = cuda.to_device(
        np.zeros((number_of_pairs, number_of_qubits), dtype=dtype)
    )
...
@cuda.jit
def _calculate_2q_marginals_from_bitstrings_kernel(
    bitstrings_array, marginals_array_out
):
    idx, idy = cuda.grid(2)
    ...
    flattened_pair_index = (
        idx * number_of_qubits - idx * (idx + 1) // 2 + (idy - idx - 1)
    )
...
def calculate_2q_marginals_from_bitstrings_cuda(
    bitstrings_array: np.ndarray, dtype=np.int32
):
    number_of_qubits = bitstrings_array.shape[1]
    number_of_pairs = number_of_qubits * (number_of_qubits - 1) // 2
    ...
    _calculate_2q_marginals_from_bitstrings_kernel[blockspergrid, threadsperblock](
        bitstrings_array, marginals_array
    )
