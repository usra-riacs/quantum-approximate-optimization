...
def _sample_from_probability_distribution_numpy_multinomial(
    ...
    samples_integers = numpy_rng.multinomial(n=number_of_samples, pvals=probabilities)
    ...
    binary_array = (
        (
            non_zero_indices[:, np.newaxis]
            & (1 << np.arange(number_of_qubits - 1, -1, -1))
        )
    ) > 0
...
def _sample_from_probability_distribution_numpy_searchsorted(
    probabilities: np.ndarray,
    number_of_samples: int,
    numpy_rng: np.random.Generator = None,
) -> np.ndarray:
...
def sample_from_probability_distribution(
    ...
    if sampling_method == "numpy_multinomial":
        return _sample_from_probability_distribution_numpy_multinomial(
            probabilities=probabilities,
            number_of_samples=number_of_samples,
            numpy_rng=numpy_rng,
        )
    elif sampling_method == "numpy_searchsorted":
        return _sample_from_probability_distribution_numpy_searchsorted(
            probabilities=probabilities,
            number_of_samples=number_of_samples,
            numpy_rng=numpy_rng,
        )
        ...
        raise ValueError(
            'sampling_method should be either "numpy_multinomial" or "numpy_searchsorted"'
        )
...
def sample_from_statevector(
    ...
    return sample_from_probability_distribution(
        probabilities=probabilities,
        number_of_samples=number_of_samples,
        numpy_rng=numpy_rng,
...
def _calculate_energies_from_bitstrings_2_local(
    bitstrings_array: Union[cp.ndarray, np.ndarray],
    ...
    products = bck.einsum(
        "ij,ij->i", bck.dot(bitstrings_array, couplings_array) / 2, bitstrings_array
    )

    if local_fields is not None:
        products += bck.dot(bitstrings_array, local_fields)
    return products


def calculate_energies_from_bitstrings_2_local(
    bitstrings_array: Union[cp.ndarray, np.ndarray],
    pm_input: bool = False,
    ...
    if adjacency_matrix is None and couplings_array is None:
        raise ValueError(
            "Either adjacency_matrix or couplings_array should be provided"
        )
    ...
    if computation_backend == "cupy":
        ...
        raise ValueError(
            f"Unknown backend_computation: {computation_backend}; should be either 'numpy' or 'cupy'"
        )
        ...
        if isinstance(adjacency_matrix, list):
            if isinstance(adjacency_matrix[0][1], tuple):
                if computation_backend == "cupy":
                    ...
                    raise ValueError(
                        'backend_computation should be either "numpy" or "cupy'
                    )
                # This is a situation when the Hamiltonian is of a form [(coeff, (i,j)), ...]
                couplings_array = bck.zeros(
                    shape=(
                ...
                if local_fields_present is None or local_fields_present:
                    local_fields = bck.zeros(
                        shape=(len(bitstrings_array[0]),), dtype=float
                    )
                else:
                    local_fields = None
    ...
    products = _calculate_energies_from_bitstrings_2_local(
...
def calculate_energies_from_bitstrings(
    ...
    arr = np.array(
        sum(
            (1 - 2 * np.bitwise_xor.reduce(comp_array[:, node_ids], axis=1))
            * coefficient
            for coefficient, node_ids in hamiltonian
        ),
        dtype=float,
    )
    ...
    if backend_output == "numpy":
        ...
        raise ValueError(
            f'backend_output should be either "numpy" or "cupy", not {backend_output}'
        )
...
try:
    ...
    def _calculate_energies_from_bitstrings_numba_kernel(
        coefficients, subsets_list, bitstrings_array
    ):
        """
    ...
    def calculate_energies_from_bitstrings_numba(
        hamiltonian: List[Tuple[float, Tuple[int, ...]]], bitstrings_array: np.ndarray
    ) -> np.ndarray:
        """
        ...
        return _calculate_energies_from_bitstrings_numba_kernel(
            coeffs, idx_lists, bitstrings_array
        )
...
def solve_hamiltonian_python(hamiltonian):
    ...
    all_bitstrings = np.array(
        list(itertools.product([0, 1], repeat=number_of_qubits)), dtype=int
    )
    all_energies = calculate_energies_from_bitstrings_2_local(
...
def convert_cupy_numpy_array(
    array: np.ndarray | cp.ndarray, output_backend: str
    ...
    if output_backend == "numpy":
        if isinstance(array, (cp.ndarray, list, tuple)):
            ...
            raise ValueError(
                f"array should be np.nddarray/cp.nddarray/list/tuple, not {type(array)}"
            )
        ...
        raise ValueError(
            f'output_backend should be either "numpy" or "cupy", not {output_backend}'
        )
...
def _get_marginal_from_probability_distribution(
    probability_distribution: Union[cp.ndarray, np.ndarray],
    subset: Union[List[int], Tuple[int]],
    number_of_qubits: int,
):

    if len(probability_distribution.shape) != number_of_qubits:
        probability_distribution = probability_distribution.copy().reshape(
            [2] * number_of_qubits
        )

    # Sum over the unwanted variables to obtain the marginal distribution
    marg = probability_distribution.sum(
        axis=tuple(i for i in range(number_of_qubits) if i not in subset)
    )
...
def get_marginals_from_probability_distribution(
    probability_distribution: Union[cp.ndarray, np.ndarray],
    ...
    for subset in subsets:
        ...
        if isinstance(key, list):
            key = tuple(key)
        all_marginals[key] = _get_marginal_from_probability_distribution(
...
def get_1q_marginals_from_bitstrings_array(
    bitstrings_array: Union[np.ndarray, cp.ndarray],
...
def get_2q_marginals_from_bitstrings_array(
    bitstrings_array: Union[np.ndarray, cp.ndarray],
...
def _get_higher_locality_marginals_from_bitstrings_array_fixed_locality(
    bitstrings_array: Union[np.ndarray, cp.ndarray],
    ...
    if subset_size == 1:
        return get_1q_marginals_from_bitstrings_array(
            bitstrings_array=bitstrings_array,
            qubits_list=[x[0] for x in qubits_subsets],
            normalize=normalize,
        )
    elif subset_size == 2:
        return get_2q_marginals_from_bitstrings_array(
            bitstrings_array=bitstrings_array,
    ...
    marginals_kq = bck.zeros(
        (len(local_register), len(qubits_subsets)), dtype=local_arrays.dtype
    )
...
def get_marginals_from_bitstrings_array(
    bitstrings_array: Union[np.ndarray, cp.ndarray],
    ...
    Union[np.ndarray, cp.ndarray],
    List[Tuple[List[Tuple[int, ...]], Union[np.ndarray, cp.ndarray]]],
]:
    ...
    assert (
        qubits_subsets is not None or qubits_subsets_by_locality is not None
    ), "At least one of qubits_subsets or qubits_subsets_by_locality must be provided"
    assert not (
        qubits_subsets is not None and qubits_subsets_by_locality is not None
    ), "Only one of qubits_subsets or qubits_subsets_by_locality must be provided"
    ...
    if len(unique_localities) == 1:
        ...
        return _get_higher_locality_marginals_from_bitstrings_array_fixed_locality(
            bitstrings_array=bitstrings_array,
    ...
    for locality, subsets_list in qubits_subsets_by_locality.items():
        marginals_l = (
            _get_higher_locality_marginals_from_bitstrings_array_fixed_locality(
                bitstrings_array=bitstrings_array,
...
if __name__ == "__main__":
    ...
    hamiltonian_adjacency_test = (
        hamiltonian_adjacency_test + hamiltonian_adjacency_test.T
    ) / 2
    ...
    energies_test = em_usra.calculate_energies_from_bitstrings_2_local(
        bitstrings_array=bitstrings_test_numpy,
        weights_matrix=hamiltonian_adjacency_test,
        computation_method=f"numpy_einsum",
    )
    t0 = time.perf_counter()
    energies_test_here = calculate_energies_from_bitstrings_2_local(
        bitstrings_array=bitstrings_test_numpy,
    ...
    energies_test_cupy = em_usra.calculate_energies_from_bitstrings_2_local(
        bitstrings_array=bitstrings_test_cupy,
    ...
    energies_test_cupy_here = calculate_energies_from_bitstrings_2_local(
        bitstrings_array=bitstrings_test_cupy,
    ...
    if correct_usra:
        anf.cool_print(
            "TEST (cupy-USRA) PASSED FOR:",
            f"{noq_test} qubits and {nos_test} samples",
            "green",
        )
    else:
        anf.cool_print(
            "TEST (cupy-USRA) FAILED FOR:",
            f"{noq_test} qubits and {nos_test} samples",
            "red",
        )
    ...
    if correct_here:
        anf.cool_print(
            "TEST (cupy-HERE) PASSED FOR:",
            f"{noq_test} qubits and {nos_test} samples",
            "green",
        )
    else:
        anf.cool_print(
            "TEST (cupy-HERE) FAILED FOR:",
            f"{noq_test} qubits and {nos_test} samples",
            "red",
        )
