...
def to_fourier_basis(gamma, beta):
    ...
    if (
        np.allclose(np.dot(A, u), gamma)
        == True & np.allclose(np.dot(B, v), beta)
        == True
    ):
        return u, v
    else:
...
def _get_gamma_beta_from_file():
    ...
    return pd.read_json(
        str(
            files("qokit.assets.maxcut_datasets").joinpath(
...
def get_best_known_parameters_for_LABS_wrt_overlap_for_p(
    N: int, p: int
