...
def _perform_sweep(
    ...
    if bias_probability != 0.0:
        biases = numpy_rng.binomial(n=1, p=bias_probability, size=len(bitstring))
...
def _perform_parallel_tempering_move(
...
def _run_parallel_tempering(
    ...
    log_probabilities_PT_all = numpy_rng.uniform(
        low=0, high=1, size=(number_of_sweeps, number_of_replicas - 1)
    )
    ...
    for sweep_index in range(number_of_sweeps):
        ...
        _perform_parallel_tempering_move(
...
class ParallelTemperingRunner:
    def __init__(
        ...
        if set(hamiltonian.localities) not in [{1}, {1, 2}, {2}]:
            raise ValueError(
                "Parallel tempering is only supported for 1- and 2-local Hamiltonians"
            )
        ...
        couplings, local_fields = hamiltonian.get_couplings_and_local_fields(
            matrix_type="SYM", backend="numpy", precision=precision
        )
    ...
    def run_parallel_tempering(
        ...
        if states_initial is not None:
            assert (
                states_initial.shape[1] == self._number_of_qubits
            ), "Initial states must have the same number of qubits as the Hamiltonian"
            if states_initial.shape[0] == 1:
                ...
                raise ValueError(
                    "The number of initial states must be equal to the number of replicas"
                )
            ...
            states_initial = 1 - 2 * self._rng.integers(
                low=0, high=1, size=(number_of_replicas, self._number_of_qubits)
            )

        if bias_probabilities is not None:
            if isinstance(bias_probabilities, float) or isinstance(
                bias_probabilities, int
            ):
                bias_probabilities = [bias_probabilities] * number_of_replicas

            assert (
                len(bias_probabilities) == number_of_replicas
        ...
        all_energies = em.calculate_energies_from_bitstrings_2_local(
        ...
        best_energy_test = em.calculate_energies_from_bitstrings_2_local(
            couplings_array=self._couplings,
            bitstrings_array=np.concatenate([all_bitstrings, 1 - all_bitstrings]),
            local_fields=self._local_fields,
        )
...
if __name__ == "__main__":
    ...
    cost_hamiltonian = generator_cost_hamiltonian.generate_instance(
    ...
    print(
        ...
        cost_hamiltonian.hamiltonian_instance_description,
    )
    ...
    ##############
    seed_test = 0
    pt_runner = ParallelTemperingRunner(
    ...
    t0 = time.perf_counter()
    (best_state, best_energy), opt_res = pt_runner.run_parallel_tempering(
        number_of_sweeps=number_of_sweeps,
    ...
    (best_state_pysa, best_energy_pysa), opt_res_pysa = (
        gsa.solve_ising_hamiltonian_with_pysa(
