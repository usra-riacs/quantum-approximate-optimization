...
class HamiltonianOptimizer:
    def __init__(
        ...
        if isinstance(input_hamiltonian_representations_cost, list):
            ...
            if isinstance(example_ham, list):
                ...
                input_hamiltonian_representations_cost = [
                    input_hamiltonian_representations_cost
                ]
            ...
            if isinstance(example_ham, list):
                number_of_qubits = (
                    max(
                        [
                            max(tup)
                            for coeff, tup in input_hamiltonian_representations_cost[0]
                        ]
                    )
                    + 1
                )
        ...
        self.update_hamiltonians_cost(
            input_hamiltonian_representations_cost, solve=solve_at_initialization
        )

        if store_n_best_results > 1:
            self._best_results_container = BestResultsContainer(
                number_of_best_results=store_n_best_results
            )
        else:
    ...
    def _update_hamiltonians_cost(
        ...
        for idx, ham in hams_range:
            ham = self._handle_hamiltonian_type(
                ham, solve=solve, number_of_qubits=self._number_of_qubits
            )
    ...
    def update_hamiltonians_cost(
        ...
        self._update_hamiltonians_cost(hamiltonian_representations_cost, solve=solve)
    ...
    def reinitialize_logger(self, *args, **kwargs):
    ...
    @staticmethod
    def _handle_hamiltonian_type(ham, solve=False, number_of_qubits=None):
        if isinstance(ham, list):
            ham = CLH(ham, number_of_qubits=number_of_qubits)
            if solve:
...
class HamiltonianSolutionsSampler(HamiltonianOptimizer):
    ...
    def sample_solutions(
        self, number_of_samples: int, *args, **kwargs
    ) -> Tuple[List[Tuple[float, _BTS_FORMAT]], Any]:

        tup_return = self._sample_solutions(
...
class EnergyResultMain:
    ...
    def to_dataframe_full(self):
        ...
        return pd.DataFrame(
            data={
                f"{SNV.EnergyMean.id_long}{SNV.Noiseless.id_long}": [
                    self.energy_mean_noiseless
                ],
                f"{SNV.EnergyMean.id_long}{SNV.Noisy.id_long}": [
                    self.energy_mean_noisy
                ],
                f"{SNV.EnergyBest.id_long}{SNV.Noiseless.id_long}": [
                    self.energy_best_noiseless
                ],
                f"{SNV.EnergyBest.id_long}{SNV.Noisy.id_long}": [
                    self.energy_best_noisy
                ],
                f"{SNV.BitstringBest.id_long}{SNV.Noiseless.id_long}": [
                    (
                        self.bitstring_best_noiseless.tolist()
                        if self.bitstring_best_noiseless is not None
                        else None
                    )
                ],
                f"{SNV.BitstringBest.id_long}{SNV.Noisy.id_long}": [
                    (
                        self.bitstring_best_noisy.tolist()
                        if self.bitstring_best_noisy is not None
                        else None
                    )
                ],
            },
        ...
        return pd.DataFrame(
            data={
                f"{SNV.EnergyMean.id_long}": [
                    float(self.energy_mean) if self.energy_mean is not None else None
                ],
                f"{SNV.EnergyBest.id_long}": [
                    float(self.energy_best) if self.energy_best is not None else None
                ],
                f"{SNV.BitstringBest.id_long}": [
                    (
                        self.bitstring_best.tolist()
                        if self.bitstring_best is not None
                        else None
                    )
                ],
            },
...
class BestResultsContainerBase:
    ...
    def add_result(self, result_to_add: Union[List[Any], Tuple[Any]], score: float):
...
class BestResultsContainer(BestResultsContainerBase):
    ...
    def add_multiple_bitstrings_results(
        self,
        bitstrings_array,
        energies_array,
        ...
        add_args = (
            ...
            if additional_global_specifiers_tuple is not None
            else ()
        )
        for x in best_n_pairs:
            ...
            res_to_add = (bts_x, add_args)

            self.add_result(score=float(energy_x), result_to_add=res_to_add)
    ...
    def add_result(self, result_to_add: Union[List[Any], Tuple[Any]], score: float):
        ...
        self.results_heap.sort(key=lambda x: x[0], reverse=False)
    ...
    def get_best_results(self) -> List[Tuple[float, Any]]:
        ...
        return [
            ...
            for score, data in sorted(
                self.results_heap, reverse=False, key=lambda x: x[0]
            )
        ]

    def add_another_heap(
        self, other_container: Union[BestResultsContainerBase, List[Tuple[float, Any]]]
    ):
        ...
        for score, data in best_results_other:
            self.add_result(score=score, result_to_add=data)
...
class HamiltonianOptimizationResultsLogger(ResultsLogger):
    ...
    def __post_init__(self):
        ...
        hamiltonian_class_description: HamiltonianOptimizationLoggerConfig = (
            self.config.CostHamiltonianClass.get_description_string()
        )
        hamiltonian_instance_description = (
            self.config.CostHamiltonianInstance.get_description_string()
        )
        shared_metadata = pd.DataFrame(
            data={
