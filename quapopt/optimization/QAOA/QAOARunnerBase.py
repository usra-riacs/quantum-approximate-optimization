...
class QAOARunnerBase(HamiltonianOptimizer):
    ...
    def reinitialize_logger(self,
        ...
        hamiltonian_class_description = cost_hamiltonian.hamiltonian_class_specifier.get_description_string()
        hamiltonian_instance_description = cost_hamiltonian.hamiltonian_instance_specifier.get_description_string()
        shared_metadata_cost = pd.DataFrame(data={"CostHamiltonianClass": [hamiltonian_class_description],
        ...
        try:
            ...
            self.results_logger.write_metadata(metadata=shared_metadata_cost,
        ...
        if phase_hamiltonians_all is not None:
            ...
            phase_hamiltonian_class_description = phase_hamiltonian.hamiltonian_class_specifier.get_description_string()
            phase_hamiltonian_instance_description = phase_hamiltonian.hamiltonian_instance_specifier.get_description_string()
            shared_metadata_phase = pd.DataFrame(data={"PhaseHamiltonianClass": [phase_hamiltonian_class_description],
                                                        "PhaseHamiltonianInstance": [phase_hamiltonian_instance_description],
                                                        })
            try:
                self.results_logger._write_shared_metadata(shared_metadata=shared_metadata_phase,
            ...
            data_phase = {f"Phase{SNV.HamiltonianRepresentationIndex.id_long}": [],
            ...
            for i, ham_i in phase_hamiltonians_all.items():
                ...
                for transformation_i in applied_transformations_i:
                    type_i = transformation_i.transformation
                    value_i = transformation_i.value
                    data_phase[f"Phase{SNV.HamiltonianRepresentationIndex.id_long}"].append(i)
                    data_phase[f"Phase{SNV.HamiltonianTransformationType.id_long}"].append(type_i.id_long)
                    data_phase[f"Phase{SNV.HamiltonianTransformationValue.id_long}"].append(value_i)
    ...
    def initialize_backend_qiskit(self,
        ...
        for i, ham_i in self.hamiltonian_representations_phase.items():
            ...
            #     #print(f"STARTED SHARED QISKIT IBM SESSION (instance {i}) (session:{session_ibm})")
