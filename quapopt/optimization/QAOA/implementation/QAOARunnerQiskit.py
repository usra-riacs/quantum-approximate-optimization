...
class QAOARunnerQiskit(QiskitSessionManagerMixin):
    ...
    def __init__(
        ...
        if qubit_mapping_type == QubitMappingType.linear_swap_network:
            tuple_0_device = tuple(
                [
                    (qubit_indices_physical[i], qubit_indices_physical[i + 1])
                    for i in range(0, self._number_of_qubits - 1, 2)
                ]
            )
            tuple_1_device = tuple(
                [
                    (qubit_indices_physical[i], qubit_indices_physical[i + 1])
                    for i in range(1, self._number_of_qubits - 1, 2)
                ]
            )

            linear_chains_pair_device = (tuple_0_device, tuple_1_device)

            ansatz = LinearSwapNetworkQAOACircuit(
                sdk_name="qiskit",
                depth=qaoa_depth,
            ...
            ansatz = FullyConnectedQAOACircuit(
                sdk_name="qiskit",
                depth=qaoa_depth,
                hamiltonian_phase=hamiltonian_phase,
                program_gate_builder=program_gate_builder,
        ...
        if qubit_mapping_type == QubitMappingType.sabre:
            ...
            for classical_index, qubit_index in zip(
                classical_indices, qubits_permutation
            ):
            ...
            if qubit_mapping_type == QubitMappingType.linear_swap_network:
                ...
                qubits_permutation = anf.reverse_permutation(
                    permutation=swap_network_permutation
                )
                qubit_indices_measurement = [
                    qubit_indices_physical[qubits_permutation[i]]
                    for i in range(len(qubits_permutation))
                ]
            else:
                qubit_indices_measurement = tuple(qubit_indices_physical)

            for classical_index, qubit_index in zip(
                classical_indices, qubit_indices_measurement
            ):
                ansatz_circuit.measure(qubit=qubit_index, cbit=classical_index)
    ...
    def remap_bitstrings(
        ...
        return anf.apply_permutation_to_array(
            array=bitstrings_array, permutation=self.bitstrings_permutation
        )

    def run_qaoa(self, angles_PHASE, angles_MIXER, number_of_samples: int):
        ...
        angles_dict = {
            self.parameters_PHASE: angles_PHASE,
            self.parameters_MIXER: angles_MIXER,
        }

        isa_circuit_resolved = self._program_gate_builder.parameters_resolver(
            quantum_circuit=self.ansatz_circuit, memory_map=angles_dict
        )
