...
def get_exp_X_operator(angle,
    ...
    return _bck.array([[_bck.cos(angle), -1j*_bck.sin(angle)],
...
if NUMBA_AVAILABLE:
    ...
    def _apply_full_qaoa_circuit_numba(input_state,
                                       angles_PS,
                                       angles_mixer,
                                       spectra_list,
                                       number_of_qubits):
            ...
            for qubit_idx in range(number_of_qubits):
                ...
                for block_start in range(0, total_dim, block_size):
                    for i in range(stride):
                        ...
                        input_state[idx_0] = cos_beta * state_0 - 1j * sin_beta * state_1
                        input_state[idx_1] = -1j * sin_beta * state_0 + cos_beta * state_1

        return input_state

    def apply_full_qaoa_circuit_numba(input_state, angles_PS, angles_mixer, batches_spectra, number_of_qubits):
        """
        ...
        return _apply_full_qaoa_circuit_numba(
            input_state, np.array(angles_PS, dtype=np.float64), np.array(angles_mixer, dtype=np.float64), spectra_list, number_of_qubits)
...
class QAOASimulatorPython:
    ...
    def get_qaoa_unitary(self,
        ...
        for layer_index, (angle_PS, angle_mixer) in tqdm(enumerate(list(zip(angles_PS[1:], angles_mixer[1:]))),disable = not show_progress_bar):
    ...
    def get_qaoa_statevector(self,
        ...
        if backend == 'cython':
            ...
            for layer_index, (angle_PS, angle_mixer) in tqdm(enumerate(list(zip(angles_PS, angles_mixer))),
                ...
                #Phase separation
                input_state = input_state * _bck.exp(-1j * angle_PS * self._batches_spectra[batch_index])
