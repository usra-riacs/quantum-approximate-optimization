...
from quapopt.data_analysis.data_handling import LoggingLevel, verify_whether_to_log_data
...
if "cuda" in AVAILABLE_SIMULATORS:
    from quapopt.optimization.QAOA.simulation.pauli_backprop.one_layer import (
        math_functions_cuda as math_p1_cuda,
    )
...
class QAOARunnerExpValues(QAOARunnerBase):
    ...
    def __init__(
        ...
        self._angles_history = {
            i: {} for i in range(len(self.hamiltonian_representations_phase))
        }

        self._constant_zeros = (
            np.zeros(self.number_of_qubits, dtype=precision_float),
            np.zeros(
                (self.number_of_qubits, self.number_of_qubits), dtype=precision_float
            ),
        )

        self._debug = False

        self._ABC_values_history = {
            i: {} for i in range(len(self.hamiltonian_representations_phase))
        }
    ...
    def update_hamiltonians_cost(
        self, hamiltonian_representations_cost: List[ClassicalHamiltonian], solve=False
    ):
        self._update_hamiltonians_cost(
            hamiltonian_representations_cost=hamiltonian_representations_cost,
            solve=False,
        )
    ...
    def _initialize_simulators_analytical(
        self, simulator_name=None, precision_float=None
    ):
        ...
        for ind, ham_cost in self.hamiltonian_representations_cost.items():
            fields_array_np, couplings_array_np = ham_cost.get_fields_and_couplings(
        ...
        if self._hamiltonian_representations_phase is not None:
            ...
            for ind, ham_phase in self.hamiltonian_representations_phase.items():
                fields_array_np, couplings_array_np = (
                    ham_phase.get_fields_and_couplings(precision=precision_float)
                )
    ...
    def _get_trigonometric_functions_arrays(
        self,
        ...
        if self.simulator_name in ["mixed", "cython"]:
            cos_correlations_phase, sin_correlations_phase = (
                math_p1_cython._get_part_1_cython(correlations_phase["numpy"], gamma)
            )

            cosine_fields = math_p1_cython._get_part_4_cython(
                fields_phase["numpy"], gamma
            )
            ...
            # TODO FBM: I think just calculating trigonometric functions is much faster with cython?
            cosine_fields = math_p1_cuda._get_part_4_cuda(
                fields_phase=fields_phase["cuda"],
                gamma=gamma,
                float_precision=self._precision_float,
            )
            ...
            raise ValueError(
                "simulator_name must be either 'cuda' or 'cython' or 'mixed'"
            )

        return cos_correlations_phase, sin_correlations_phase, cosine_fields

    def _get_product_formula_array(
        self, correlations_cost, correlations_phase, cos_correlations_phase, gamma
    ):

        if self.simulator_name in ["mixed", "cuda"]:
            product_formulas_array = math_p1_cuda._get_part_3_cuda(
            ...
            product_formulas_array = math_p1_cython._get_part_3_cython(
    ...
    def _get_operators_dict(
        ...
        if store_products_in_memory:
            if gamma not in self._angles_history[hamiltonian_representation_index]:
                cos_correlations_phase, sin_correlations_phase, cosine_fields = (
                    self._get_trigonometric_functions_arrays(
                ...
                product_formulas_array = self._get_product_formula_array(
                ...
                self._angles_history[hamiltonian_representation_index][gamma] = [
                    cos_correlations_phase,
                    sin_correlations_phase,
                    cosine_fields,
                    product_formulas_array,
                ]
            cos_correlations_phase = self._angles_history[
                hamiltonian_representation_index
            ][gamma][0]
            sin_correlations_phase = self._angles_history[
                hamiltonian_representation_index
            ][gamma][1]
            cosine_fields = self._angles_history[hamiltonian_representation_index][
                gamma
            ][2]
            product_formulas_array = self._angles_history[
                hamiltonian_representation_index
            ][gamma][3]
        ...
        if self._debug:
            ...
            anf.cool_print(
                "RATIO MEMORY TRIG/PRODUCT:", total_memory_trig / memory_product, "blue"
            )
        ...
        if add_ABC:
            ...
            if 1 in self.hamiltonian_representations[0].localities:
                if store_products_in_memory:
                    ...
                    if (
                        gamma
                        not in self._angles_history[hamiltonian_representation_index]
                    ):
                        _evaluate_sines = True
                    elif (
                        len(
                            self._angles_history[hamiltonian_representation_index][
                                gamma
                            ]
                        )
                        < 5
                    ...
                    if _evaluate_sines:
                        if self.simulator_name in ["cython", "mixed"]:
                            sine_fields_phase = math_p1_cython._get_sine_fields_cython(
                                fields_phase["numpy"], gamma
                            )
                        else:
                            raise NotImplementedError(
                                "This method is not implemented for local fields yet"
                            )
                        self._angles_history[hamiltonian_representation_index][
                            gamma
                        ].append(sine_fields_phase)

                    sine_fields_phase = self._angles_history[
                        hamiltonian_representation_index
                    ][gamma][4]
                else:
                    if self.simulator_name in ["cython", "mixed"]:
                        ...
                        sine_fields_phase = math_p1_cuda._get_sine_fields_cuda(
                            fields_phase=fields_phase["cuda"],
                            gamma=gamma,
                            float_precision=self._precision_float,
                        )
                ...
                if fields_cost is None:
                    fields_cost = self.fields_cost_dict[
                        hamiltonian_representation_index
                    ]
                ...
                if self.simulator_name in ["cuda", "mixed"]:
                    Ci = math_p1_cuda._get_part_2_cuda(
                        fields_phase=fields_phase["cuda"],
                    ...
                    Ci = math_p1_cython._get_part_2_cython(
                        fields_phase["numpy"],
                        fields_cost["numpy"],
                        correlations_phase["numpy"],
                        cos_correlations_phase,
            ...
            if self.simulator_name in ["cuda", "mixed"]:
                BC_values = math_p1_cuda._get_BC_cuda(
                    fields_phase=fields_phase["cuda"],
                    correlations_cost=correlations_cost["cuda"],
                    sin_correlations_phase=sin_correlations_phase,
                    cosine_fields=cosine_fields,
                ...
                BC_values = math_p1_cython._get_BC_cython(
                    fields_phase["numpy"],
                    correlations_cost["numpy"],
                    sin_correlations_phase,
                    cosine_fields,
    ...
    def _get_expected_values(
        ...
        if simulator_name in ["cuda", "mixed"]:
            ...
            Cij = math_p1_cuda._get_part_5_cuda(
                fields_phase=fields_phase["cuda"],
                correlations_cost=correlations_cost["cuda"],
                sin_correlations_phase=sin_correlations_phase,
                cosine_fields=cosine_fields,
            ...
            Cij = math_p1_cython._get_part_5_cython(
                fields_phase["numpy"],
                correlations_cost["numpy"],
                sin_correlations_phase,
                cosine_fields,
    ...
    def update_history(self, qaoa_result: QAOAResult):
        ...
        self._best_results_container.add_result(
            result_to_add=tup_to_store, score=best_energy
        )

    def log_results(self, qaoa_result: QAOAResult):
        ...
        self.results_logger.write_results(
            dataframe=optimization_overview_df, data_type=optimization_overview_dt
        )

        correlators_dt = SNDT.Correlators

        if verify_whether_to_log_data(
            data_type=correlators_dt, logging_level=self.results_logger._logging_level
        ):
            ...
            self.results_logger.write_results(
                dataframe=qaoa_result.correlators, data_type=correlators_dt
            )

    def get_best_results(self):
        return self._best_results_container.get_best_results()

    def _input_handler_analytical_betas_p1(
        self,
        args,
        ...
        if input_format in [FIFormat.direct_full]:
            ...
            if len(self.hamiltonian_representations) > 1:
                ...
                assert (
                    len(args) == 2
                ), "The number of angles must be equal to 1 for analytical betas."
            else:
            ...
            assert (
                len(angles_gamma) == 1
            ), "The number of angles_gamma must be equal to 1 for analytical betas"
            ...
            bounds_optuna_transformations = [
                (
                    SNV.HamiltonianRepresentationIndex.id,
                    tuple(range(len(self.hamiltonian_representations))),
                )
            ]
    ...
    def run_qaoa(
        ...
        *args,
        ...
        assert (
            number_of_samples is None or number_of_samples == np.inf
        ), "This method is only implemented for number_of_samples=None or np.inf"

        time.perf_counter()
        if analytical_betas:
            angles, hamiltonian_representation_index, trial_index = (
                self._input_handler_analytical_betas_p1(
                    args=args, input_format=input_format
                )
            )
        ...
        if analytical_betas:
            if 1 in self.hamiltonian_representations[0].localities:
                ...
                self._ABC_values_history[hamiltonian_representation_index][
                    trial_index
                ] = np.array([A, B, C])
                ...
                new_betas_list = [root for root in roots_arccos] + [
                    -root for root in roots_arccos
                ]
                ...
                roots_alt = np.array(
                    [r for r in roots_alt if abs(r.imag) <= 10 ** (-9)]
                )
                roots_alt = np.round(roots_alt.real, 10)
                roots_arctan = 1 / 2 * np.arctan(roots_alt)
                [root for root in roots_arctan] + [-root for root in roots_arctan]
                ...
                for new_beta in new_betas_list:
                    ...
                    new_args = (gamma_j, new_beta, hamiltonian_representation_index)
                ...
                final_args = (gamma_j, best_beta, hamiltonian_representation_index)
                ...
                new_args = (gamma_j, beta_main, hamiltonian_representation_index)
        ...
        if measurement_noise is None:
            ...
            Ci_noiseless, Cij_noiseless = self._get_expected_values(
                gamma=gamma_j,
                beta=beta_j,
                fields_phase=fields_phase,
            ...
            if not store_correlators:
                ...
                exp_value_noiseless = np.sum(Ci_noiseless) + np.sum(
                    np.sum(Cij_noiseless)
                )
        ...
        fields_cost = measurement_noise.noisy_hamiltonian_fields["cost"][
            hamiltonian_representation_index
        ]
        correlations_cost = measurement_noise.noisy_hamiltonian_couplings["cost"][
            hamiltonian_representation_index
        ]
        ...
        Ci_noisy, Cij_noisy = self._get_expected_values(
            gamma=gamma_j,
            beta=beta_j,
            fields_phase=fields_phase,
            fields_cost=fields_cost,
