...
@cuda.jit
def _product_kernel_main_local_terms(
    fields_phase,
    fields_cost,
    correlations_phase,
...
@cuda.jit
def _kernel_cosine_and_sine_correlations(
    correlations_phase, gamma, cos_correlations_phase_out, sin_correlations_phase_out
):
...
@cuda.jit
def _kernel_cosine_vector_fields(fields_phase, gamma, cosine_fields_out):
    i = cuda.grid(1)
    if i >= fields_phase.shape[0]:
        return
...
@cuda.jit
def _kernel_sine_vector_fields(fields_phase, gamma, sine_fields_out):
    i = cuda.grid(1)
    if i >= fields_phase.shape[0]:
        return
...
@cuda.jit
def _kernel_product_formulas_C_ij(
    correlations_cost,
    correlations_phase,
    cos_correlations_phase,
    gamma,
    ...
    # First term
    product1, product2 = 1.0, 1.0
    # Second term
    product3, product4 = 1.0, 1.0
    # Third term
    product5, product6 = 1.0, 1.0
    for k in range(n):
        ...
        if correlations_phase[ind_1, k] != 0 and correlations_phase[ind_2, k] != 0:
            product5 *= np.cos(
                gamma * (correlations_phase[ind_1, k] + correlations_phase[ind_2, k])
            )
            product6 *= np.cos(
                gamma * (correlations_phase[ind_1, k] - correlations_phase[ind_2, k])
            )
...
def _kernel_calculate_C_ij(
    ...
    C *= (
        np.cos(gamma * (h_i_phase + h_j_phase)) * product5
        - np.cos(gamma * (h_i_phase - h_j_phase)) * product6
    )
...
def _get_part_1_cuda(correlations_phase, gamma, float_precision=np.float64):
    gamma = 2 * gamma
    ...
    _kernel_cosine_and_sine_correlations[blocks_per_grid, threads_per_block](
        correlations_phase, gamma, cos_correlations_phase, sin_correlations_phase
    )
...
def _get_part_2_cuda(
    ...
    _product_kernel_main_local_terms[blocks_per_grid, threads_per_block](
        fields_phase,
        fields_cost,
        correlations_phase,
        cos_correlations_phase,
...
def _get_part_3_cuda(
    ...
    _kernel_product_formulas_C_ij[blocks_per_grid, threads_per_block](
        correlations_cost,
        correlations_phase,
        cos_correlations_phase,
        gamma,
...
def _get_part_4_cuda(fields_phase, gamma, float_precision=np.float64):
    gamma = 2 * gamma
    ...
    _kernel_cosine_vector_fields[blocks_per_grid, threads_per_block](
        fields_phase, gamma, cosine_fields
    )
...
def _get_sine_fields_cuda(fields_phase, gamma, float_precision=np.float64):
    gamma = 2 * gamma
    number_of_qubits = fields_phase.shape[0]
    n = number_of_qubits
    ...
    _kernel_sine_vector_fields[blocks_per_grid, threads_per_block](
        fields_phase, gamma, sine_fields
    )
...
def _get_part_5_cuda(
    ...
    sin_4_beta_by_2 = np.sin(4.0 * beta) / 2
    sin_2_beta_squared_by_2 = (np.sin(2.0 * beta) ** 2) / 2
    ...
    _kernel_calculate_C_ij[blocks_per_grid, threads_per_block](
        fields_phase,
        correlations_cost,
        sin_correlations_phase,
        cosine_fields,
...
def _get_BC_cuda(
    ...
    _kernel_calculate_BC[blocks_per_grid, threads_per_block](
        fields_phase,
        correlations_cost,
        sin_correlations_phase,
        cosine_fields,
...
def analytical_QAOA_cuda(
    ...
    time.perf_counter()
    cos_correlations_phase, sin_correlations_phase = _get_part_1_cuda(
        correlations_phase,
        correlations_cost,
        fields_phase,
        fields_cost,
    ...
    time.perf_counter()
    Ci_expected_values = _get_part_2_cuda(
        fields_phase,
        fields_cost,
        correlations_phase,
        cos_correlations_phase,
    ...
    time.perf_counter()
    product_formulas_array = _get_part_3_cuda(
        correlations_cost,
        correlations_phase,
        cos_correlations_phase,
        gamma,
    ...
    time.perf_counter()
    cosine_fields = _get_part_4_cuda(
        fields_phase, gamma, float_precision=float_precision
    )
    time.perf_counter()

    C_ij_expected_values = _get_part_5_cuda(
        fields_phase,
        correlations_cost,
        sin_correlations_phase,
        cosine_fields,
