...
def analytical_qaoa_python(
    ...
    for i in range(number_of_qubits):
        cosine_fields[i] = np.cos(2.0 * gamma * fields_phase[i])
        sine_fields[i] = np.sin(2.0 * gamma * fields_phase[i])
        for j in range(i + 1, number_of_qubits):
            if (
                correlations_cost[i, j] == 0
                and fields_cost[i] == 0
                and fields_phase[i] == 0
            ):
                continue
            cos_2_gamma = np.cos(2.0 * gamma * correlations_phase[i, j])
            cosine_correlations[i, j] = cos_2_gamma
            cosine_correlations[j, i] = cos_2_gamma

            sin_2_gamma = np.sin(2.0 * gamma * correlations_phase[i, j])
    ...
    for i in range(number_of_qubits):
        ...
        for k in range(number_of_qubits):
            ...
            product *= cos_2_gamma
        if product != 0:
            C_i_expected_values[i] = (
                h_i_cost * sin_2beta * np.sin(2.0 * gamma * h_i_phase) * product
            )
    ...
    for i in range(number_of_qubits):
        ...
        for j in range(i + 1, number_of_qubits):
            ...
            for k in range(number_of_qubits):
                ...
                if correlations_phase[i, k] != 0 and correlations_phase[j, k] != 0:
                    product5 *= np.cos(
                        2.0
                        * gamma
                        * (correlations_phase[i, k] + correlations_phase[j, k])
                    )
                    product6 *= np.cos(
                        2.0
                        * gamma
            ...
            term3 = (
                np.cos(2.0 * gamma * (h_i_phase + h_j_phase)) * product5
                - np.cos(2.0 * gamma * (h_i_phase - h_j_phase)) * product6
            )
...
def _get_ABC_direct_python(
    local_fields: np.ndarray, couplings: np.ndarray, gamma: float
):
    ...
    cosine_correlations = np.ones(
        (number_of_qubits, number_of_qubits), dtype=local_fields.dtype
    )
    sine_correlations = np.zeros(
        (number_of_qubits, number_of_qubits), dtype=local_fields.dtype
    )
    ...
    for q_u in range(number_of_qubits):
        ...
        for q_v in range(q_u + 1, number_of_qubits):
            ...
            for q_w in range(number_of_qubits):
                ...
                if (
                    couplings[q_v, q_w] != 0
                    and couplings[q_w, q_u] != 0
                    and q_w != q_v
                    and q_w != q_u
                ):
                    if prod_f_plus != 0.0:
                        cos_uv_f_plus = cos(
                            gamma * (couplings[q_w, q_u] + couplings[q_w, q_v])
                        )
                        if cos_uv_f_plus == 0:
                            prod_f_plus = 0
                        else:
