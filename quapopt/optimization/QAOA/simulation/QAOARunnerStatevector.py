...
class QAOARunnerStatevector(QAOARunnerBase):
    ...
    def log_results(self, qaoa_result: QAOAResult):
        ...
        statevector_df = pd.DataFrame(
            data={SNDT.StateVectors.id_long: [qaoa_result.statevector]}
        )
        statevector_dt = SNDT.StateVectors

        self.results_logger.write_results(
            dataframe=qaoa_result.annotate_dataframe(statevector_df),
    ...
    def run_qaoa(
        ...
        *args,
        ...
        if hamiltonian_i.spectrum is None:
            self._hamiltonian_representations_cost[
                hamiltonian_representation_index
            ].solve_hamiltonian()
            hamiltonian_i = self._hamiltonian_representations_cost[
                hamiltonian_representation_index
        ...
        if self._backend_name.lower() in ["qokit"]:
            ...
            _result = simulator_i.get_qaoa_statevector(
                angles_PS=gammas_j, angles_mixer=betas_j
            )
        ...
        statevector_ideal = anf.convert_cupy_numpy_array(
            array=statevector_ideal, output_backend="numpy"
        )
        prob_distro_ideal = em.cython_abs_squared(statevector_ideal.reshape(-1))
        exp_value_noiseless = em.cython_vdot(
            vector1=prob_distro_ideal.astype(np.float32),
        ...
        if measurement_noise.noisy_hamiltonian_representations["cost"] is not None:
            ...
            hamiltonian_noisy, noisy_constant = (
                measurement_noise.transform_hamiltonian_to_noisy_version(
                    hamiltonian_list=noiseless_hamiltonian
                )
            )

        if measurement_noise.noisy_hamiltonian_spectra["cost"] is not None:
            noisy_spectrum = measurement_noise.noisy_hamiltonian_spectra["cost"][
                hamiltonian_representation_index
            ]
        else:
            ...
            simulator_noisy = simclass_qokit_noisy(
                self.number_of_qubits, terms=hamiltonian_noisy
            )
        ...
        exp_value_noisy = (
            em.cython_vdot(
                vector1=prob_distro_ideal.astype(np.float32),
                vector2=noisy_spectrum.astype(np.float32),
            )
            + noisy_constant
        )
