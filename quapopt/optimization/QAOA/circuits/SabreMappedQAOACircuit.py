...
class SabreMappedQAOACircuit(MappedAnsatzCircuit):
    ...
    def __init__(
        ...
        program_gate_builder=NativeGateBuilderHeronCustomizable(
        ...
        if hamiltonian_phase.is_two_local:
            ...
            ansatz_qiskit = FullyConnectedQAOACircuit(
                sdk_name="qiskit",
                depth=depth,
            ...
            ansatz_qiskit, parameters = build_qiskit_qaoa_ansatz(
                depth=depth,
                hamiltonian_phase=hamiltonian_phase,
                time_block_size=time_block_size,
                phase_separator_type=phase_separator_type,
        ...
        qubits_physical_indices = (
            bck_utils.get_nontrivial_physical_indices_from_circuit(
                quantum_circuit=circuit_qiskit_isa
            )
        )

        mapping_original_to_final = bck_utils.get_physical_qubits_mapping_from_circuit(
            quantum_circuit=circuit_qiskit_isa
        )

        qubit_mapping_permutation = tuple(
            [mapping_original_to_final[i] for i in qubit_indices_original]
        )
...
def get_qaoa_ansatz_qiskit_router(
    ...
    if not assert_no_ancilla_qubits:
        raise NotImplementedError(
            "THis function is not implemented for circuits with ancilla qubits yet. "
        )
    ...
    if assert_no_ancilla_qubits:
        assert phase_hamiltonian.number_of_qubits == number_of_qubits_circuit, (
            f"Exceeded maximal number of "
            f"tries for flag "
...
if __name__ == "__main__":
    ...
    permutation_1_test = get_linear_chain_permutation(
        swap_chain=swap_chain_1_test, number_of_qubits=number_of_qubits_test
    )
    permutation_2_test = get_linear_chain_permutation(
        swap_chain=swap_chain_2_test, number_of_qubits=number_of_qubits_test
    )
    ...
    depth_test = 1
    swap_network_permutation_test = get_swap_network_permutation(
    ...
    coeffs = list(
        [
            int("".join([str(x) for x in sorted([i, j], reverse=True)]))
            for (i, j) in pairs_test
        ]
    )
    hamiltonian_list_representation = [
        (coeff / 2, pair) for coeff, pair in zip(coeffs, pairs_test)
    ]
    hamiltonian_list_representation += [
        ((i + 1) / 2, (i,)) for i in range(number_of_qubits_test)
    ]

    print({pair: coeff * 2 for coeff, pair in hamiltonian_list_representation})

    hamiltonian_test = ClassicalHamiltonian(
    ...
    circuit_builder_LSN = LinearSwapNetworkQAOACircuit(
        sdk_name="qiskit",
        depth=depth_test,
    ...
    hamiltonian_matrix_diag = (
        em.get_matrix_representation_of_classical_hamiltonian_diag(
    ...
    for angles_index in tqdm(list(range(random_angles.shape[1]))):
        ...
        executable_qiskit = program_gate_builder_test.resolve_parameters(
            quantum_circuit=quantum_circuit_qiskit, memory_map=memory_map
        )
