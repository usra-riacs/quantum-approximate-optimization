...
def build_qiskit_qaoa_ansatz(  # sdk_name: str,
    ...
    assert (
        phase_separator_type == PhaseSeparatorType.QAOA
    ), "Only QAOA phase separator is supported for now. "
    ...
    if time_block_size != 1.0:
        ...
        number_of_interactions_per_batch = int(
            np.ceil(number_of_interactions / number_of_batches)
        )
        ...
        for qi in range(number_of_qubits):
            if initial_state == "|+>" or initial_state is None:
                ...
                raise NotImplementedError(
                    "Only |+> initial state is supported for now. "
                )

        for param_index in range(depth):
            batch_index = param_index % number_of_batches

            hamiltonian_batch = hamiltonian_phase_list[
                batch_index
                * number_of_interactions_per_batch : (batch_index + 1)
                * number_of_interactions_per_batch
            ]
            if len(hamiltonian_batch) == 0:
                continue

            hamiltonian_batch_qiskit = (
                bck_utils.convert_hamiltonian_list_representation_to_qiskit_observable(
                    hamiltonian_list_representation=hamiltonian_batch,
                    number_of_qubits=hamiltonian_phase.number_of_qubits,
                )
            )

            ansatz_qiskit = QiskitQAOAAnsatz(
                cost_operator=hamiltonian_batch_qiskit,
                reps=1,
            ...
            ansatz_qiskit.assign_parameters(
                parameters={
                    beta_batch: angle_mixer[param_index],
        ...
        ansatz_qiskit = QiskitQAOAAnsatz(
            cost_operator=qiskit_hamiltonian,
            reps=depth,
        ...
        ansatz_qiskit.assign_parameters(parameters=params_dict, inplace=True)
...
if __name__ == "__main__":
    ...
    ham_simple = [
        ...
        for ind, (i, j) in enumerate(
            [(k, l) for k in range(noq_test) for l in range(k + 1, noq_test)]
        )
    ]
    print(ham_simple)
    test_ansatz = build_qiskit_qaoa_ansatz(
        hamiltonian_phase=ClassicalHamiltonian(
            hamiltonian_list_representation=ham_simple, number_of_qubits=noq_test
        ),
        depth=1,
        time_block_size=1.0,
        phase_separator_type=PhaseSeparatorType.QAOA,
