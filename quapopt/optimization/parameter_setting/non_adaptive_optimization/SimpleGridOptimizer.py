...
class SimpleGridOptimizer(NonAdaptiveOptimizer):
    def __init__(
        ...
        non_fixed_parameters = [
            x for x in parameter_bounds if x[0] != ParametersBoundType.CONSTANT
        ]
        categorical_parameters = [
            x[1] for x in non_fixed_parameters if x[0] == ParametersBoundType.SET
        ]
        total_mult_factor_categorical = np.prod(
            [len(x) for x in categorical_parameters]
        )

        assert (
            total_mult_factor_categorical <= max_trials
        ), "Number of trials is too small for minimal grid size."

        continuous_parameters = [
            x for x in non_fixed_parameters if x[0] == ParametersBoundType.RANGE
        ]
        ...
        if number_of_continuous_parameters != 0:
            size_each_continuous_grid = int(
                trials_left_for_continuous ** (1 / number_of_continuous_parameters)
            )
        else:
        ...
        for i in range(number_of_parameters):
            ...
            if bound_type == ParametersBoundType.RANGE:
                ...
                local_search_spaces.append(
                    np.linspace(
                        start=min_value, stop=max_value, num=size_each_continuous_grid
                    )
                )
