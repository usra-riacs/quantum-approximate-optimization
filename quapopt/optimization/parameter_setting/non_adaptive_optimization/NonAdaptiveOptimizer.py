...
class NonAdaptiveOptimizer:
    ...
    def _run_optimization(
        ...
        for trial_index, arguments in tqdm(
            list(dict(enumerate(self.search_space[0:real_number_of_trials])).items()),
            colour="yellow",
            position=0,
            ...
            if abs(funval) >= 10**10:
                raise ValueError(
                    f"Function value is too large: {funval} for arguments: {arguments}"
                )
            ...
            if funval < best_funval:
                if verbosity > 0:
                    print(
                        f"New minimum found at trial: {trial_index} with function value: {best_funval} -> {funval}"
                    )
        ...
        data = {
            "TrialIndex": list(range(real_number_of_trials)),
            "FunctionValue": function_values_list,
        }
        for i in range(len(self.argument_names)):
            data[self.argument_names[i]] = [
                x[i] for x in self.search_space[0:real_number_of_trials]
            ]

        df = pd.DataFrame(data)

        return OptimizationResult(
            best_value=best_funval, best_arguments=best_arguments, trials_dataframe=df
        )
