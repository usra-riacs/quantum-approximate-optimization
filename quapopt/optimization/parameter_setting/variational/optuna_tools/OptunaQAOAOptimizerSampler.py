...
def get_2q_rdms_diag(Zij_values, Zi_values):
    rdms_2q_list = []
...
class OptunaQAOAOptimizerSampler:
    def __init__(
        ...
        raise DeprecationWarning(
            "This class is deprecated. Use QAOAOptimizationRunner instead."
        )
    ...
    def run_optimization(
        ...
        **additional_kwargs,
        ...
        if weird_version_temp:
            hamiltonian_dict = {
                tup: coeff
                for coeff, tup in self.qaoa_sampler.hamiltonian_representations[
                    0
        ...
        def __objective_function(trial: optuna.Trial):
            ...
            if not weird_version_temp:
                return self._qaoa_sampler.run_qaoa_wrapped(
                    trial,
            ...
            energy_mean = self._qaoa_sampler.run_qaoa_wrapped(
                trial,
                qaoa_depth=qaoa_depth,
                number_of_samples=number_of_samples,
                measurement_noise=measurement_noise,
        ...
        if classical_optimizer is None:
            classical_optimizer = optuna.samplers.TPESampler(
                seed=optimizer_seed, **__default_hyperparameters_TPE__
            )
        if optuna_pruner is None:
            optuna_pruner = optuna.pruners.NopPruner()

        optuna_study = optuna.create_study(
            sampler=classical_optimizer,
...
class OptunaQAOAOptimizerSamplerQRR:
    def __init__(
        ...
        raise NotImplementedError(
            "This class is deprecated. Will make QRR external to the QAOA sampler."
        )

        super().__init__(
            qaoa_sampler=qaoa_sampler,
    ...
    def run_optimization(
        ...
        if weird_version_temp:
            hamiltonian_dict = {
                tup: coeff
                for coeff, tup in self.qaoa_sampler.hamiltonian_representations[0]
            }
        ...
        for hamiltonian_tup, trials_i in all_correlators.items():
            ...
            hamiltonian_i = self.qaoa_sampler.hamiltonian_representations[
                hamiltonian_index
            ]
            for trial_index, correlators in trials_i.items():
                if measurement_noise is None:
                    C_i, C_ij = (
                        correlators["Ci_noiseless"],
                ...
                C_ij_weighted = QRR_functions.add_weights_to_correlations_matrix(
                    correlations_matrix=C_ij,
                ...
                if add_noisy_constant:
                    noisy_constant = (
                        measurement_noise.noisy_hamiltonian_representations["cost"][
                            hamiltonian_index
                        ][1]
                    )
                    noisy_constant /= self.qaoa_sampler.number_of_qubits

                candidate_solutions = QRR_functions.find_candidate_solutions_QRR(
                    expected_values=flatten_upper_triangle,
                    qubit_subsets=indices_subsets,
                    include_1q_terms=include_1q_terms,
                    return_correlations_matrix=False,
                    constant=noisy_constant,
                )
                candidate_solutions_weighted = (
                    QRR_functions.find_candidate_solutions_QRR(
                        expected_values=flatten_upper_triangle_weighted,
                        qubit_subsets=indices_subsets,
                        include_1q_terms=include_1q_terms,
                        return_correlations_matrix=False,
                        constant=noisy_constant,
                ...
                join_candidate_solutions = np.concatenate(
                    (candidate_solutions, candidate_solutions_weighted), axis=0
                )

                candidate_energies = hamiltonian_i.evaluate_energy(
                    bitstrings_array=join_candidate_solutions
                )
