...
def _cupy_old_version_replace_put_along_axis(
    arr, indices, values, axis, expanded_indices=None
):
    """
...
class RandomLocalSearchRunner(NDARRunner):
    def __init__(
        ...
        if self._hamiltonian.default_backend != backend:
            print(
                "CHANGING BACKEND:", f"{self._hamiltonian.default_backend} -> {backend}"
            )
        ...
        super().__init__(
            input_hamiltonian_representations=[self._hamiltonian],
            attractor_model=None,
            convergence_criterion=convergence_criterion,
        )

    def _sample_new_solutions(
        self,
    ...
    def run_NDAR(
        ...
        total_number_of_bytes_bitstrings = (
            number_of_samples_per_trial * number_of_trials_max * self._number_of_qubits
        )
        ...
        best_energy_so_far = self._hamiltonian.evaluate_energy(
            bitstrings_array=[bitstring_mask]
        )[0]
        ...
        ind_i = -1
        while (
            not self.check_convergence() and ndar_iteration_index < number_of_trials_max
        ):
            ...
            if (
                samples_investigated_so_far % batch_size_bitstrings
                < number_of_samples_per_trial
            ):
                if verbosity >= 2:
                    print(
                        "TRIAL NUMBER:", ndar_iteration_index, ", GENERATING NEW BATCH"
                    )
                t0 = time.perf_counter()

                t1 = time.perf_counter()
                random_bitstrings_flipped = rng_uniform.binomial(
                    n=1,
                    p=1 / 2 * (1 - p_01),
                    size=(batch_size_bitstrings, self._number_of_qubits),
                )
            ...
            random_bitstrings_flipped_i = random_bitstrings_flipped[
                ind_i : ind_i + number_of_samples_per_trial, :
            ]
            if len(random_bitstrings_flipped_i) < number_of_samples_per_trial:
                additional_samples_i = number_of_samples_per_trial - len(
                    random_bitstrings_flipped_i
                )
                random_additional_i = rng_uniform.binomial(
                    n=1,
                    p=1 / 2 * (1 - p_01),
                    size=(additional_samples_i, self._number_of_qubits),
                )

                random_bitstrings_flipped_i = bck.concatenate(
                    [random_bitstrings_flipped_i, random_additional_i], axis=0
                )
            elif len(random_bitstrings_flipped_i) > number_of_samples_per_trial:
                random_bitstrings_flipped_i = random_bitstrings_flipped_i[
                    :number_of_samples_per_trial, :
                ]

            t02 = time.perf_counter()
            random_bitstrings_flipped_i_masked = (
                random_bitstrings_flipped_i ^ bitstring_mask
            )
            flipped_energies = self._hamiltonian.evaluate_energy(
                bitstrings_array=random_bitstrings_flipped_i_masked,
            ...
            self._print_energy_progress_nicely(
                current_energy=best_energy_i,
            ...
            if add_GA:
                pairs_bts_energy = bck.hstack(
                    (
                        random_bitstrings_flipped_i_masked,
                        flipped_energies.reshape(-1, 1),
                    )
                )
                pairs_bts_energy = pairs_bts_energy[
                    bck.argsort(pairs_bts_energy[:, -1])
                ]
                best_solutions_n = pairs_bts_energy[: kwargs_GA["sol_per_pop"], 0:-1]

                (solution_ga, energy_ga), ga_instance_i = (
            ...
            self._optimization_history[ndar_iteration_index] = (
                best_energy_i,
                best_bitstring_i,
            )
    ...
    def run_fixed_HW_NDAR(
        ...
        assert (
            hamming_weight > 0
        ), "Please provide a positive integer for hamming weight"
        ...
        if bck == cp:
            ...
            expanded_indices_batch = cp.ogrid[
                tuple(map(slice, (batch_size_bitstrings, self._number_of_qubits)))
            ]
        ...
        while (
            ...
            if (
                ...
                random_bitstrings_flipped = bck.zeros(
                    (batch_size_bitstrings, self._number_of_qubits), dtype=int
                )
                random_indices_i = rng_choice.choice(
                    a=all_qubits,
                ...
                if bck == np:
                    bck.put_along_axis(
                        random_bitstrings_flipped, random_indices_i, 1, axis=1
                    )
                elif bck == cp:
                    _cupy_old_version_replace_put_along_axis(
                        random_bitstrings_flipped,
                        random_indices_i,
                        1,
                        axis=1,
            ...
            if len(random_bitstrings_flipped_i) < number_of_samples_per_trial:
                ...
                random_additional_i = bck.zeros(
                    (additional_samples_i, self._number_of_qubits), dtype=int
                )
                random_indices_i = rng_choice.choice(
                    a=all_qubits,
                ...
                if bck == np:
                    ...
                    _cupy_old_version_replace_put_along_axis(
                        random_additional_i, random_indices_i, 1, axis=1
                    )
                else:
                    raise ValueError("Backend not recognized")
