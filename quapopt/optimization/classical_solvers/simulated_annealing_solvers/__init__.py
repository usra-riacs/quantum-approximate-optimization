...
from quapopt.additional_packages.pysa.ParallelTemperingRunner import ParallelTemperingRunner


def _solve_ising_hamiltonian_parallel_tempering(hamiltonian:ClassicalHamiltonian,
    ...
    runner = ParallelTemperingRunner(hamiltonian=hamiltonian,
                                     rng_seed=solver_seed,
                                     precision=precision_float)

    (best_state, best_energy), df_res = runner.run_parallel_tempering(number_of_sweeps=number_of_sweeps,
...
def solve_ising_hamiltonian_pysa(hamiltonian:ClassicalHamiltonian,
                                number_of_sweeps: int,
                                temperatures: List[float],
                                states_initial: Optional[np.ndarray] = None,
                                qubits_orderings: Optional[Union[List[List[int]], List[int]]] = None,
    ...
    if solver_name == 'PT':
        return _solve_ising_hamiltonian_parallel_tempering(hamiltonian=hamiltonian,
