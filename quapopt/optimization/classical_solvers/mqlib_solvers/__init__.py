...
from quapopt.hamiltonians.representation.problem_formulations import (
    ProblemFormulationType,
    _calculate_ising_objective_direct,
    map_adjacency_between_formulations,
    map_maxcut_solution_to_ising,
)


def solve_maxcut_problem_with_mqlib(
    ...
    mqlib_instance = MQLib.Instance(problem="M", dat=dat)
    # MQLIB MAXIMIZES <x|Q|x> if QUBO is given.
    mqlib_result = MQLib.runHeuristic(
        heuristic=solver_name,
...
def solve_ising_hamiltonian_mqlib(
    hamiltonian: Union[
        List[Tuple[float, Tuple[int, ...]]], np.ndarray, ClassicalHamiltonian
    ],
    solver_kwargs=None,
    ...
    if isinstance(hamiltonian, list):
        adjacency_matrix = convert_list_representation_to_adjacency_matrix(
            hamiltonian,
            matrix_type="SYM",
            backend="numpy",
        ...
        adjacency_matrix = hamiltonian.get_adjacency_matrix(
    ...
    adjacency_matrix_maxcut = map_adjacency_between_formulations(
        input_adjacency=sign * adjacency_matrix,
        input_formulation=ProblemFormulationType.ISING,
        output_formulation=ProblemFormulationType.MAXCUT,
    )

    res_mqlib = solve_maxcut_problem_with_mqlib(
        adjacency_matrix=adjacency_matrix_maxcut, **solver_kwargs
    )
    ...
    ising_energy = em.calculate_energies_from_bitstrings_2_local(
        bitstrings_array=np.array([ising_solution]),
...
if __name__ == "__main__":
    ...
    for seed_test in range(0, 10):
        ...
        if input_matrix_name == ProblemFormulationType.ISING:
            energies_bruteforce = em.calculate_energies_from_bitstrings_2_local(
        ...
        (best_bitstring_output, best_energy_output), _ = solve_ising_hamiltonian_mqlib(
            hamiltonian=input_matrix,
            solver_kwargs={
                "solver_name": "BURER2002",
                "solver_timeout": 0.1,
        ...
        if input_matrix_name in [ProblemFormulationType.ISING]:
            best_energy_output = _calculate_ising_objective_direct(
                adjacency_matrix=input_matrix, bitstrings_array=[best_bitstring_output]
            )[0]
        else:
            raise ValueError(
                "Input matrix name should be either 'ISING', 'QUBO' or 'MAXCUT"
