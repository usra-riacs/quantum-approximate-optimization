...
def apply_bitflip_to_hamiltonian(
    hamiltonian: Union[
        networkit_Graph, List[Tuple[Union[float, int], Tuple[int, ...]]]
    ],
    ...
    assert not np.any(bitflip_set < 0) and not np.any(
        bitflip_set > 1
    ...
    if isinstance(hamiltonian, networkit_Graph):
        ...
        transformed_graph = nk.Graph(number_of_nodes, weighted=True)
        ...
        for edge, original_weight in hamiltonian.iterEdgesWeights():
            ...
            if node_i == node_j:
                ...
                sign_change = _SMALL_REGISTERS[2][
                    (bitflip_tuple[node_i], bitflip_tuple[node_j])
                ]
            if sign_change:
        ...
        for interaction in hamiltonian:
            ...
            sign_change = _SMALL_REGISTERS[len(node_ids)][
                tuple(bitflip_tuple[qi] for qi in node_ids)
            ]
            if sign_change:
...
def apply_permutation_to_array(
    permutation: Tuple[pyd.conint(ge=0), ...], array_to_permute: np.ndarray
) -> np.ndarray:
    """
...
def apply_permutation_to_list(
    permutation: Tuple[pyd.conint(ge=0), ...],
    list_to_permute: Union[list, tuple, np.ndarray],
    ...
    if isinstance(list_to_permute, np.ndarray):
        return apply_permutation_to_array(
            permutation=permutation, array_to_permute=list_to_permute
        )
...
def multiply_permutations(
    permutations_to_multiply: Tuple[Tuple[pyd.conint(ge=0), ...]],
) -> Tuple[pyd.conint(ge=0), ...]:
...
def apply_bitflip_to_bitstrings_array(
    bitflip_tuple: Tuple[pyd.conint(ge=0, le=1), ...], bitstrings_array: np.ndarray
) -> np.ndarray:
    """
    ...
    def __apply_xor(bts):
        return np.bitwise_xor(bts, bitflip_tuple)

    flipped_array = np.apply_along_axis(__apply_xor, 1, bitstrings_array)
    return flipped_array.astype(int)


def apply_bitflip_to_list(
    bitflip_tuple: Tuple[pyd.conint(ge=0, le=1), ...],
    list_of_bits: Union[list, tuple, np.ndarray],
) -> Union[list, tuple, np.ndarray]:
    """
    ...
    if isinstance(list_of_bits, np.ndarray):
        return apply_bitflip_to_bitstrings_array(
            bitflip_tuple=bitflip_tuple, array_to_flip=list_of_bits
        )

    flipped_list = [
        int(not bit) if flip else bit for bit, flip in zip(list_of_bits, bitflip_tuple)
    ]
...
def apply_permutation_to_hamiltonian(
    hamiltonian: Union[
        networkit_Graph, List[Tuple[Union[float, int], Tuple[int, ...]]]
    ],
    permutation_tuple: Tuple[pyd.conint(ge=0), ...],
    ...
    if isinstance(hamiltonian, networkit_Graph):
        assert set(permutation_tuple) == set(
            range(hamiltonian.numberOfNodes())
        ), "Invalid permutation tuple, it should be a tuple of all integers from 0 to n-1"
        ...
        for edge, original_weight in hamiltonian.iterEdgesWeights():
            ...
            transformed_graph.addEdge(node_i_permuted, node_j_permuted, original_weight)
...
def concatenate_hamiltonian_transformations(
    ...
    for transformation_full in transformations_list:
        transformation, value = (
            transformation_full.transformation,
            transformation_full.value,
        )
        if isinstance(transformation, SNV.Bitflip):
            value_permuted = apply_permutation_to_list(
                permutation=invert_permutation(permutation_combined),
                list_to_permute=bitflip_combined,
            )
            bitflip_combined = apply_bitflip_to_list(
                bitflip_tuple=value, list_of_bits=value_permuted
            )
        elif isinstance(transformation, SNV.Permutation):
            permutation_combined = multiply_permutations(
                permutations_to_multiply=(permutation_combined, value)
            )
        else:
...
def create_mapping(ham_prob, all_pairs_ansatz):
