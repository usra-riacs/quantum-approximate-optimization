...
class RandomClassicalHamiltonianGeneratorBase:
    def __init__(self,
       ...
       # assert number_of_qubits > 0, "Number of qubits must be positive."
        ...
        if hamiltonian_class_specifier is None:
            ...
            hamiltonian_class_specifier = HamiltonianClassSpecifierGeneral(hamiltonian_model_name=hamiltonian_model_name,
    ...
    def _generate_instance(self,
        ...
        #class_description, instance_description = self.get_class_and_instance_descriptions(hamiltonian_instance_index=seed)
        ...
        #     #     return numpy_rng.uniform() < self._average_degree / self._max_degree
        #
        #     def term_addition_function(numpy_rng) -> bool:
        #         return True
...
if __name__ == '__main__':
    ...
    RCHGB = RandomClassicalHamiltonianGeneratorBase(hamiltonian_class_specifier=hamiltonian_spec)

    number_of_qubits = 10000
    random_hamiltonian = RCHGB._generate_instance(seed=42,
    ...
    energies_2 = em.calculate_energies_from_bitstrings(observable=random_hamiltonian.hamiltonian,
                                                       measurement_values=random_bitstrings,
                                                       )
    print('done with numpy')
    t2 = time.perf_counter()
    energies_3 = em.cuda_calculate_energies_from_bitstrings(hamiltonian=random_hamiltonian.hamiltonian,
                                                            bitstrings=random_bitstrings,
                                                            d_energies=d_energies)
    print('done with cuda')
    t3 = time.perf_counter()
    energies_4 = em.cython_calculate_energies_from_bitstrings(hamiltonian=random_hamiltonian.hamiltonian,
                                                              bitstrings=random_bitstrings,
                                                              )
