...
from quapopt.hamiltonians.generators.RandomClassicalHamiltonianGeneratorBase import (
    ...
    _get_default_coefficient_sampling_function,
)
...
class RandomErdosRenyiHamiltonianGenerator(RandomClassicalHamiltonianGeneratorBase):
    def __init__(
        ...
        assert set(localities) in [
            {2},
            {1, 2},
        ], "Localities must be either (2,) or (1, 2)."

        if hamiltonian_class_specifier is None:
            # generic ER graph
            hamiltonian_class_specifier = HamiltonianClassSpecifierErdosRenyi(
    ...
    def generate_instance(
        ...
        hamiltonian_class_specifier: HamiltonianClassSpecifierErdosRenyi = (
            self._hamiltonian_class_specifier
        )
        ...
        if erdos_renyi_type == ERDOS_RENYI_TYPES.Gnp:
            ...
            subsets_generator_function = itertools.combinations(
                range(number_of_qubits), 2
            )
            probability_of_adding_edge = p_or_M

            def term_addition_function(numpy_rng, size: int):
                return (
                    numpy_rng.uniform(size=size, low=0.0, high=1.0)
                    < probability_of_adding_edge
                )
            ...
            def subsets_generator_function(numpy_rng):
                subsets_generator = list(
                    itertools.combinations(range(number_of_qubits), 2)
                )
        ...
        _coefficient_sampling_function = _get_default_coefficient_sampling_function(
        ...
        return ClassicalHamiltonian(
            hamiltonian_list_representation=terms_1q
            + ham.hamiltonian_list_representation,
