...
from quapopt.data_analysis.data_handling import HamiltonianClassSpecifierMAXkSAT

# TODO(FBM): refactor those functions


def get_mapping_2SAT(clause: ClauseFormat, qubit_index: int) -> int:
    """
...
def get_local_fields_from_2SAT_clauses(
    qubit_index: int, clauses_list: List[ClauseFormat]
):
    """
    ...
    """
    return -sum(
        [get_mapping_2SAT(clause=cl, qubit_index=qubit_index) for cl in clauses_list]
    )


def get_interaction_coefficients_from_2SAT_clauses(
    pair_indices: Tuple[int, int], clauses_list: List[ClauseFormat]
):
    """
    ...
    return sum(
        [
            get_mapping_2SAT(clause=cl, qubit_index=pair_indices[0])
            * get_mapping_2SAT(clause=cl, qubit_index=pair_indices[1])
            for cl in clauses_list
        ]
    )


def generate_random_2SAT_clauses(
...
def convert_2SAT_clauses_to_ising(
    clauses_list: List[ClauseFormat], number_of_qubits: int
    ...
    hamiltonian_list = []
    for qi in tqdm(
        list(range(number_of_qubits)), position=0, colour="blue", disable=True
    ):
        h_i = get_local_fields_from_2SAT_clauses(
            qubit_index=qi, clauses_list=clauses_list
        )
        if h_i != 0:
            hamiltonian_list.append((h_i, (qi,)))

        for qj in range(qi + 1, number_of_qubits):
            J_ij = get_interaction_coefficients_from_2SAT_clauses(
                pair_indices=(qi, qj), clauses_list=clauses_list
            )
...
try:
    ...
    @cuda.jit
    def _get_local_fields_from_2SAT_clauses_kernel_cuda(clauses_list, local_fields):
    ...
    @cuda.jit
    def _get_interaction_coefficients_from_2SAT_clauses_kernel_cuda(
        clauses_list, interaction_coefficients
    ):
...
def get_local_fields_from_2SAT_clauses_cuda(
    clauses_list: List[ClauseFormat], number_of_qubits: int
):
    """
    Convert 2SAT clauses to local fields
    ...
    _get_local_fields_from_2SAT_clauses_kernel_cuda[1, number_of_qubits](
        clauses_list_device, local_fields_device
    )

    return -local_fields_device.copy_to_host()


def get_interaction_coefficients_from_2SAT_clauses_cuda(
    clauses_list: List[ClauseFormat], number_of_qubits: int
):
    """
    Convert 2SAT clauses to ZZ interactions
    ...
    interaction_coefficients_device = cuda.to_device(
        np.zeros((number_of_qubits, number_of_qubits), dtype=np.float64)
    )
    clauses_list_device = cuda.to_device(clauses_list)
    ...
    _get_interaction_coefficients_from_2SAT_clauses_kernel_cuda[
        blocks_per_grid, threads_per_block
    ](clauses_list_device, interaction_coefficients_device)
    ...
    def generate_instance(
        ...
        hamiltonian_instance_specifier = (
            hamiltonian_class_specifier.instance_specifier_constructor(
                number_of_qubits, seed, clause_density
            )
        )
        ...
        clauses_list = generate_random_2SAT_clauses(
        ...
        if number_of_qubits > 150 and "cuda" in AVAILABLE_SIMULATORS:
            local_fields = get_local_fields_from_2SAT_clauses_cuda(
                clauses_list=clauses_list, number_of_qubits=number_of_qubits
            )
            interaction_coefficients = (
                get_interaction_coefficients_from_2SAT_clauses_cuda(
                    clauses_list=clauses_list, number_of_qubits=number_of_qubits
                )
            )
            hamiltonian_list = []
            for qi in tqdm(
                list(range(number_of_qubits)), position=0, colour="green", disable=True
            ):
                h_i = local_fields[qi]
                if h_i != 0:
                    hamiltonian_list.append((h_i, (qi,)))
            ...
            hamiltonian_list = convert_2SAT_clauses_to_ising(
                clauses_list=clauses_list, number_of_qubits=number_of_qubits
            )
...
if __name__ == "__main__":
    ...
    RM2SHG = RandomMAX2SATHamiltonianGenerator(
        number_of_variables=number_of_variables_test,
