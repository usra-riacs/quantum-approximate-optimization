...
from quapopt.hamiltonians.generators.LABSHamiltonianGenerator import (
...
from quapopt.hamiltonians.generators.RandomMaxCutHamiltonianGenerator import (
    RandomMaxCutHamiltonianGenerator,
)
from quapopt.hamiltonians.generators.RandomRegularHamiltonianGenerator import (
...
from quapopt.hamiltonians.generators.RandomSherringtonKirkpatrickHamiltonianGenerator import (
    RandomSKHamiltonianGenerator,
)
from quapopt.hamiltonians.generators.RandomWishartHamiltonianGenerator import (
...
def build_hamiltonian_generator(
    hamiltonian_model: HamiltonianModels,
    coefficients_distribution_specifier=None,
    localities=None,
    erdos_renyi_type: ERDOS_RENYI_TYPES = None,
) -> Union[
    RandomSKHamiltonianGenerator,
    RandomRegularHamiltonianGenerator,
    RandomErdosRenyiHamiltonianGenerator,
    ...
    if hamiltonian_model in [
        HamiltonianModels.SherringtonKirkpatrick,
        HamiltonianModels.RegularGraph,
    ]:
        arguments_dict_builder["coefficients_distribution_specifier"] = (
...
def create_hamiltonian_from_descriptions(
    ...
    from quapopt.data_analysis.data_handling import (
        reconstruct_hamiltonian_class_specifier,
        reconstruct_hamiltonian_instance_specifier,
    )

    # Reconstruct the class and instance specifiers
    class_specifier = reconstruct_hamiltonian_class_specifier(class_description)
    instance_specifier = reconstruct_hamiltonian_instance_specifier(
        instance_description, class_specifier
    )
    ...
    coefficients_distribution_specifier = (
        class_specifier.CoefficientsDistributionSpecifier
    )
    ...
    if (
        hasattr(instance_specifier, "EdgeProbabilityOrAmount")
        and "p_or_M" in sig.parameters
    ):
...
if __name__ == "__main__":
    ...
    cost_hamiltonian_class_description_test = (
        cost_hamiltonian_test.hamiltonian_class_description
    )
    cost_hamiltonian_instance_description_test = (
        cost_hamiltonian_test.hamiltonian_instance_description
    )
    ...
    cost_hamiltonian_reconstructed_test = create_hamiltonian_from_descriptions(
        class_description=cost_hamiltonian_class_description_test,
        instance_description=cost_hamiltonian_instance_description_test,
        read_from_drive_if_present=True,
        default_backend="numpy",
    )
    cost_hamiltonian_class_description_test_reconstructed = (
        cost_hamiltonian_reconstructed_test.hamiltonian_class_description
    )
    cost_hamiltonian_instance_description_test_reconstructed = (
        cost_hamiltonian_reconstructed_test.hamiltonian_instance_description
    )
    print(
        "Reconstructed class description (cost):",
        cost_hamiltonian_class_description_test_reconstructed,
    )
    print(
        "Reconstructed instance description (cost):",
        cost_hamiltonian_instance_description_test_reconstructed,
    )
    same_class_test = (
        cost_hamiltonian_class_description_test
