...
class ConvergenceCriterion(StandardizedSpecifier):
    ...
    def __init__(
        ...
        if convergence_value is None:
            if convergence_criterion_name == ConvergenceCriterionNames.MaxIterations:
                ...
                convergence_criterion_name
                == ConvergenceCriterionNames.MaxUnsuccessfulTrials
            ):
                convergence_value = 5
            elif (
                convergence_criterion_name == ConvergenceCriterionNames.BestEnergyChange
            ):
                convergence_value = 10**-6
            elif (
                convergence_criterion_name == ConvergenceCriterionNames.MeanEnergyChange
            ):
                convergence_value = 10**-6
            else:
                raise ValueError(
                    f"Value needs to be provided for: {convergence_criterion_name}"
                )
    ...
    def check_convergence(
        self, previous_score=None, current_score=None, iteration_index=None
    ):
        ...
        if (
            convergence_criterion == ConvergenceCriterionNames.MaxIterations
            ...
            convergence_criterion == ConvergenceCriterionNames.BestEnergyChange
            ...
            raise NotImplementedError(
                f"Convergence criterion: {convergence_criterion} is not implemented"
            )
...
class AttractorModel(StandardizedSpecifier):
    ...
    def __init__(
        ...
        if attractor_state is None:
            if attractor_state_type == AttractorStateType.zero:
                ...
                raise ValueError(
                    f"You have to provide attractor state for {attractor_state_type}"
                )
    ...
    def with_new_attractor_state(
        self, attractor_state: Tuple[int, ...]
    ) -> "AttractorModel":
    ...
    def return_bitflip_transformation(
        self, bitstring: Union[Tuple[int, ...], np.ndarray]
    ):
        ...
        if self.AttractorStateType == AttractorStateType.zero:
        ...
        elif self.AttractorStateType in [
            AttractorStateType.adaptive,
            AttractorStateType.custom,
        ]:
            ...
            return tuple(
                [
                    0 if attractor_state[i] == bitstring[i] else 1
