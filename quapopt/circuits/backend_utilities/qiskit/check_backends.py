...
try:
    ...
    _backend_aer_gpu = AerSimulator(device="GPU", method="statevector")

    _backend_aer_cpu = AerSimulator(device="CPU", method="statevector")
    ...
    _circ.measure(0, 0)
    try:
        _job_gpu = _sampler_aer_gpu.run([_circ], shots=10)
    ...
    try:
        _job_cpu = _sampler_aer_cpu.run([_circ], shots=10)
