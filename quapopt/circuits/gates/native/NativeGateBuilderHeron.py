...
class NativeGateBuilderHeronCustomizable(AbstractProgramGateBuilder):
    ...
    def __init__(
        ...
        super().__init__(sdk_name="qiskit", native_gate_set=native_gate_set)
        ...
        if random_delays_kwargs is not None:
            ...
            _delays_distribution_properties = random_delays_kwargs.get(
            ...
            if _delays_distribution_properties is None:
                ...
                _delays_distribution_properties = {"mean": 0.01, "std": 0.01}

            else:
                assert (
                    "mean" in _delays_distribution_properties
                ), "Delays distribution properties must contain 'mean' key."
                assert (
                    "std" in _delays_distribution_properties
                ), "Delays distribution properties must contain 'std' key."
    ...
    def _maybe_add_random_delay(
        self, circuit: QuantumCircuit, qubits: List[int]
    ) -> QuantumCircuit:
        ...
        delays = self._numpy_rng.normal(
            loc=distro_mean, scale=distro_std, size=len(qubits)
        )
    ...
    def _I(self):
        ...
        circuit = self._maybe_add_random_delay(circuit=circuit, qubits=[0])

        return circuit

    def _RZ(self, angle: AngleQiskit) -> QuantumCircuit:
        circuit = QuantumCircuit(1, 1)
    ...
    def _CZ(self) -> QuantumCircuit:
        ...
        circuit = self._maybe_add_random_delay(circuit=circuit, qubits=[0, 1])

        return circuit

    ########### NATIVE GATE SET FOR FRACTIONAL GATES CONTINUES HERE
    def _RX(self, angle: AngleQiskit) -> QuantumCircuit:
        circuit = QuantumCircuit(1, 1)
        ...
        if self.use_fractional_gates:
            ...
            circuit = self.exp_X(
                quantum_circuit=circuit, angles_tuple=[angle / 2], qubits_tuple=[0]
            )

        return circuit

    def _RZZ(self, angle: AngleQiskit) -> QuantumCircuit:
        circuit = QuantumCircuit(2, 2)
        if self.use_fractional_gates:
    ...
    def _H(self) -> QuantumCircuit:
        ...
        # this decomposition is the same for fractional and non-fractional gates
        circuit = self.RZ(
            quantum_circuit=circuit, angles_tuple=[np.pi / 2], qubits_tuple=[0]
        )
        circuit = self.SX(quantum_circuit=circuit, qubits_tuple=[0])
        circuit = self.RZ(
            quantum_circuit=circuit, angles_tuple=[np.pi / 2], qubits_tuple=[0]
        )
        return circuit
    ...
    def _Y(self) -> QuantumCircuit:
        ...
        circuit = self.RZ(
            quantum_circuit=circuit, angles_tuple=[np.pi], qubits_tuple=[0]
        )
        circuit = self.X(quantum_circuit=circuit, qubits_tuple=[0])
    ...
    def _T(self) -> QuantumCircuit:
        ...
        circuit = self.RZ(
            quantum_circuit=circuit, angles_tuple=[np.pi / 4], qubits_tuple=[0]
        )
    ...
    def _exp_X(self, angle: AngleQiskit) -> QuantumCircuit:
        ...
        if self.use_fractional_gates:
            circuit = self.RX(
                quantum_circuit=circuit, angles_tuple=[angle], qubits_tuple=[0]
            )
        else:
            circuit = self.RZ(
                quantum_circuit=circuit, angles_tuple=[np.pi / 2], qubits_tuple=[0]
            ...
            circuit = self.SX(quantum_circuit=circuit, qubits_tuple=[0])
            circuit = self.RZ(
                quantum_circuit=circuit, angles_tuple=[angle + np.pi], qubits_tuple=[0]
            )
            circuit = self.SX(quantum_circuit=circuit, qubits_tuple=[0])
            circuit = self.RZ(
                quantum_circuit=circuit, angles_tuple=[5 * np.pi / 2], qubits_tuple=[0]
            )
    ...
    def _exp_Y(self, angle: AngleQiskit) -> QuantumCircuit:
        ...
        if self.use_fractional_gates:
            ...
            circuit = self.SX(quantum_circuit=circuit, qubits_tuple=[0])
            circuit = self.RZ(
                quantum_circuit=circuit, angles_tuple=[3 * np.pi], qubits_tuple=[0]
            )
    ...
    def _exp_XX(self, angle: AngleQiskit) -> QuantumCircuit:
        ...
        if self.use_fractional_gates:
            ...
            # q0 and q1
            circuit = self.RZ(
                quantum_circuit=circuit, angles_tuple=[np.pi / 2], qubits_tuple=[0, 1]
            )
            circuit = self.SX(quantum_circuit=circuit, qubits_tuple=[0, 1])
            circuit = self.RZ(
                quantum_circuit=circuit, angles_tuple=[np.pi], qubits_tuple=[0, 1]
            )
    ...
    def _exp_ZZ(self, angle: AngleQiskit) -> QuantumCircuit:
        ...
        if self.use_fractional_gates:
            ...
            # q1
            circuit = self.RZ(
                quantum_circuit=circuit, angles_tuple=[np.pi / 2], qubits_tuple=[1]
            )
            circuit = self.SX(quantum_circuit=circuit, qubits_tuple=[1])
            # CZ
            circuit = self.CZ(quantum_circuit=circuit, qubits_pairs_tuple=[(0, 1)])
            # q1
            circuit = self.RZ(
                quantum_circuit=circuit, angles_tuple=[np.pi], qubits_tuple=[1]
            )

            circuit = self.SX(quantum_circuit=circuit, qubits_tuple=[1])

            circuit = self.RZ(
                quantum_circuit=circuit, angles_tuple=[np.pi + angle], qubits_tuple=[1]
            )
    ...
    def _exp_ZZ_SWAP(
        self, angle: AngleQiskit, compilation_variant=None
    ) -> QuantumCircuit:
        ...
        if self.use_fractional_gates:
            ...
            if compilation_variant == 0:
                ...
                raise NotImplementedError(
                    "This compilation variant is not implemented yet."
                )
...
class NativeGateBuilderHeron(NativeGateBuilderHeronCustomizable):
    ...
    def __init__(self, use_fractional_gates: bool = False):
        super().__init__(
