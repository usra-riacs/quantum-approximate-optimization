...
from sympy import Mul, Number, Symbol
...
try:
    ...
    from qiskit.circuit import Parameter as ParameterQiskit
...
try:
    ...
    from pyquil.quilatom import MemoryReference as ParameterPyquil
    from pyquil.quilatom import Mul
...
QubitsPairsTuple = Union[
    Tuple[Tuple[conint(ge=0), conint(ge=0)], ...],
    List[Tuple[conint(ge=0), conint(ge=0)]],
]

DecompositionDict = Dict[str, Dict[str, List[Union[NodeId, EdgeId]]]]

GateBuilderTypeQiskit = Union[
    Callable[[], CircuitQiskit], Callable[[AngleQiskit, ...], CircuitQiskit]
]

GateBuilderTypePyquil = Union[
    Callable[[QubitsTuple], CircuitPyquil],
    Callable[[QubitsTuple, AnglePyquil, ...], CircuitPyquil],
]
GateBuilderTypeCirq = Union[
    Callable[[QubitsTuple], CircuitCirq],
    Callable[[QubitsTuple, AngleCirq, ...], CircuitCirq],
]

AbstractGateBuilderType = Union[
    GateBuilderTypeQiskit, GateBuilderTypePyquil, GateBuilderTypeCirq
]
...
def add_gate_qiskit(
    ...
    return quantum_circuit.compose(gate_builder(*arguments), qubits=qubits)


def add_gate_pyquil(
    quantum_circuit: CircuitPyquil,
...
def add_measurements_pyquil(
    ...
    try:
        ...
        ro = MemoryReference(
            name=ro_declaration.name, declared_size=ro_declaration.memory_size
        )
    except KeyError:
    ...
    for classical_index, physical_index in zip(classical_indices, qubit_indices):
        program += MEASURE(qubit=physical_index, classical_reg=ro[classical_index])
    return program


def add_measurements_qiskit(
    quantum_circuit: CircuitQiskit,
    qubit_indices: Tuple[int],
    classical_indices: Tuple[int] = None,
...
def resolve_parameters_cirq(
    quantum_circuit: CircuitCirq, memory_map: Dict[AngleCirq, float]
):
    return cirq.resolve_parameters(quantum_circuit, memory_map)


def resolve_parameters_qiskit(
    quantum_circuit: CircuitQiskit, memory_map: Dict[AngleQiskit, float]
):
    return quantum_circuit.assign_parameters(parameters=memory_map)


def resolve_parameters_pyquil(
    quantum_circuit: CircuitPyquil, memory_map: Dict[AnglePyquil, float]
):
    # quantum_circuit.
...
def combine_circuits_qiskit(
    left_circuit: CircuitQiskit,
    right_circuit: CircuitQiskit,
    pass_manager: PassManagerQiskit = None,
) -> CircuitQiskit:
...
def combine_circuits_pyquil(
    left_circuit: CircuitPyquil, right_circuit: CircuitPyquil
) -> CircuitPyquil:
    return left_circuit + right_circuit


def combine_circuits_cirq(
    left_circuit: CircuitCirq, right_circuit: CircuitCirq
) -> CircuitCirq:
    return left_circuit + right_circuit
...
class AbstractProgramGateBuilder:
    def __init__(self, sdk_name: str, native_gate_set: Optional[List[str]] = None):
        assert (
            sdk_name.lower() in _SUPPORTED_SDKs
        ), f"The only supported SDKs are: {_SUPPORTED_SDKs}"
    ...
    def repeat_circuit(
        self, quantum_circuit: AbstractCircuit, repeats: int
    ) -> AbstractCircuit:

        repeated_circuit = self.copy_circuit(quantum_circuit=quantum_circuit)
        for _ in range(repeats - 1):
            repeated_circuit = self.combine_circuits(
                left_circuit=repeated_circuit, right_circuit=quantum_circuit
            )
        return repeated_circuit

    def get_circuit_adjoint(self, quantum_circuit: AbstractCircuit):
    ...
    def resolve_parameters(self, quantum_circuit: AbstractCircuit, memory_map: Dict):

        quantum_circuit = quantum_circuit.copy()

        return self.parameters_resolver(
            quantum_circuit=quantum_circuit, memory_map=memory_map
        )

    def combine_circuits(
        self, left_circuit: AbstractCircuit, right_circuit: AbstractCircuit
    ):
        return self.circuits_combiner(
            left_circuit=left_circuit, right_circuit=right_circuit
        )

    def copy_circuit(self, quantum_circuit: AbstractCircuit):
        return self.circuit_copier(quantum_circuit=quantum_circuit)

    def compose_multiple_gates(
        self,
        ...
        for node_or_edge, angles, some_kwargs in zip(
            targets_tuple, angles_tuple, other_kwargs_tuple
        ):
            ...
            quantum_circuit = self.circuit_composer(
                quantum_circuit=quantum_circuit,
    ...
    def H(
        ...
        return self.compose_multiple_gates(
            gate_builder=self._H,
            quantum_circuit=quantum_circuit,
    ...
    def exp_ZZ_SWAP(
        ...
        return self.compose_multiple_gates(
            gate_builder=self._exp_ZZ_SWAP,
            quantum_circuit=quantum_circuit,
            targets_tuple=qubits_pairs_tuple,
            angles_tuple=angles_tuple,
            other_kwargs_tuple=other_kwargs_tuple,
    ...
    def exp_ZZXXYY_SWAP(
        ...
        return self.compose_multiple_gates(
            gate_builder=self._exp_ZZXXYY_SWAP,
            quantum_circuit=quantum_circuit,
            targets_tuple=qubits_pairs_tuple,
            angles_tuple=angles_tuple,
            other_kwargs_tuple=other_kwargs_tuple,
    ...
    def SX(
        ...
        return self.compose_multiple_gates(
            gate_builder=self._SX,
            quantum_circuit=quantum_circuit,
            targets_tuple=qubits_tuple,
            other_kwargs_tuple=other_kwargs_tuple,
        )

    def _H(self, *arguments) -> AbstractCircuit:
        raise NotImplementedError(
            f"{self.__class__.__name__} has not implemented _H()."
        )

    def _X(self, *arguments) -> AbstractCircuit:
        raise NotImplementedError(
            f"{self.__class__.__name__} has not implemented _X()."
        )

    def _Y(self, *arguments) -> AbstractCircuit:
        raise NotImplementedError(
            f"{self.__class__.__name__} has not implemented _Y()."
        )

    def _Z(self, *arguments) -> AbstractCircuit:
        raise NotImplementedError(
            f"{self.__class__.__name__} has not implemented _Z()."
        )

    def _I(self, *arguments) -> AbstractCircuit:
        raise NotImplementedError(
    ...
    def _exp_Z(self, *arguments) -> AbstractCircuit:
        raise NotImplementedError(
            f"{self.__class__.__name__} has not implemented _exp_Z()."
        )

    def _exp_XXYY(self, *arguments) -> AbstractCircuit:
        raise NotImplementedError(
            f"{self.__class__.__name__} has not implemented _exp_XXYY()."
        )

    def _exp_XX(self, *arguments) -> AbstractCircuit:
        raise NotImplementedError(
            f"{self.__class__.__name__} has not implemented _exp_XX()."
        )

    def _exp_YY(self, *arguments) -> AbstractCircuit:
        raise NotImplementedError(
            f"{self.__class__.__name__} has not implemented _exp_YY()."
        )

    def _exp_ZZ(self, *arguments) -> AbstractCircuit:
        raise NotImplementedError(
            f"{self.__class__.__name__} has not implemented _exp_ZZ()."
        )

    def _exp_ZZ_SWAP(self, *arguments) -> AbstractCircuit:
        raise NotImplementedError(
            f"{self.__class__.__name__} has not implemented _exp_ZZ_SWAP()."
        )

    def _exp_ZZXXYY(self, *arguments) -> AbstractCircuit:
        raise NotImplementedError(
            f"{self.__class__.__name__} has not implemented _exp_ZZXXYY()."
        )

    def _exp_ZZXXYY_SWAP(self, *arguments) -> AbstractCircuit:
        raise NotImplementedError(
            f"{self.__class__.__name__} has not implemented _exp_ZZXXYY_SWAP()."
        )

    def _XX(self, *arguments) -> AbstractCircuit:
        raise NotImplementedError(
            f"{self.__class__.__name__} has not implemented _XX()."
        )

    def _YY(self, *arguments) -> AbstractCircuit:
        raise NotImplementedError(
            f"{self.__class__.__name__} has not implemented _YY()."
        )

    def _ZZ(self, *arguments) -> AbstractCircuit:
        raise NotImplementedError(
            f"{self.__class__.__name__} has not implemented _ZZ()."
        )

    def _SWAP(self, *arguments) -> AbstractCircuit:
        raise NotImplementedError(
            f"{self.__class__.__name__} has not implemented _SWAP()."
        )

    def _CNOT(self, *arguments) -> AbstractCircuit:
        raise NotImplementedError(
            f"{self.__class__.__name__} has not implemented _SWAP()."
        )

    # optional gates for some specific experiments
    def _CZ(self, *arguments) -> AbstractCircuit:
        pass

    def _RZZ(self, *arguments) -> AbstractCircuit:
        pass

    def _RZ(self, *arguments) -> AbstractCircuit:
        pass

    def _RX(self, *arguments) -> AbstractCircuit:
        pass

    def _SX(self, *arguments) -> AbstractCircuit:
        pass

    def _exp_ZZZZ(self, *arguments) -> AbstractCircuit:
        pass
