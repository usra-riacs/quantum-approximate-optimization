...
class DelaySchedulerNone(DelaySchedulerBase):
    ...
    def __init__(self):
        super().__init__(
            delay_type=DelayScheduleType.NONE,
            delay_unit="ns",
...
class DelaySchedulerLinear(DelaySchedulerBase):
    ...
    def __init__(
        ...
        super().__init__(
            delay_type=DelayScheduleType.LINEAR,
            delay_unit="ns",
    ...
    def generate_delays(self, number_of_delays: int):
        ...
        if self._indices_ordering is None:
            ...
            return [
                self.delay_base * self._indices_ordering[i]
...
class DelaySchedulerRandom(DelaySchedulerBase):
    ...
    def __init__(
        ...
        """
        super().__init__(
            delay_type=DelayScheduleType.RANDOM,
            delay_unit="ns",
            delay_at_the_end=delay_at_the_end,
            add_barriers_to_layers=add_barriers_to_layers,
        )
        ...
        self._distribution_description = CoefficientsDistributionSpecifier(
            ...
            CoefficientsDistributionProperties=self._coefficients_distribution_properties,
        ).get_description_string()

    def generate_delays(self, number_of_delays: int):
        if self._coefficients_distribution == CoefficientsDistribution.Uniform:
            delays = self._numpy_rng.uniform(
                low=self._coefficients_distribution_properties["low"],
            ...
            delays = self._numpy_rng.normal(
                loc=self._coefficients_distribution_properties["mean"],
            ...
            delays = np.full(
                shape=number_of_delays,
                fill_value=self._coefficients_distribution_properties["value"],
            )
        else:
            raise ValueError(
                f"Unknown CoefficientsDistributionName: {self._coefficients_distribution}"
            )
    ...
    def get_string_description(self):
        return (
            f"{self.get_string_description_base()};"
            f"RNGSeed={self._rng_seed};"
            f"Dist={self._distribution_description}"
        )


def add_delays_to_circuit_layers(
    ...
    quantum_circuit_delayed = QuantumCircuit(
        quantum_circuit.num_qubits, number_of_qubits
    )

    for circuit_layer in dag_circuit.layers():
        circuit_layer = dag_to_circuit(circuit_layer["graph"])
        circuit_layer_delayed = QuantumCircuit(
            quantum_circuit.num_qubits, number_of_qubits
        )
        delays_here = delay_scheduler.generate_delays(
            number_of_delays=len(circuit_layer.data)
        )
        if delays_here is None:
            delays_here = [0.0] * len(circuit_layer.data)

        for idx_instr, (delay_length, instr) in enumerate(
            zip(delays_here, circuit_layer.data)
        ):
            qubits_instr = instr.qubits
            circ_instr = QuantumCircuit.from_instructions(
                instructions=[instr], qubits=qubits_instr
            )

            if delay_length != 0.0:
                if (
                    for_visualization
                    and delay_scheduler.delay_type == DelayScheduleType.LINEAR
                ):
                    # Linear schedule can be visualized nicely.
                    for qubit in qubits_instr:
                        for _ in range(idx_instr):
                            circuit_layer_delayed.delay(
                                duration=delay_length / idx_instr,
                                qarg=qubit,
            ...
            circuit_layer_delayed = circuit_layer_delayed.compose(
                circ_instr, inplace=False, qubits=qubits_instr
            )
        ...
        quantum_circuit_delayed = quantum_circuit_delayed.compose(
            circuit_layer_delayed, inplace=False, qubits=circuit_layer_delayed.qubits
        )

    if not ignore_delay_at_the_end:
        if delay_scheduler.delay_at_the_end > 0.0:
            nontrivial_qubits = bck_utils.get_nontrivial_physical_indices_from_circuit(
                quantum_circuit=quantum_circuit_delayed
            )
            for qubit in nontrivial_qubits:
                quantum_circuit_delayed.delay(
                    duration=delay_scheduler.delay_at_the_end,
                    qarg=qubit,
                    unit=delay_scheduler.delay_unit,
                )
