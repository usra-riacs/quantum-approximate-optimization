...
class CAFAnalyzer:
    def __init__(
        ...
        if results_logger_kwargs is None:
            ...
            assert results_list is None, (
                "If results logger is provided, " "the results list should be None."
            )
            ...
            for circuit_index in unique_jobs:
                ...
                assert (
                    len(unique_delay_schedules) == 1
                ), "The delays should be unique for each circuit index."
                assert (
                    len(unique_mcr) == 1
                ), "The mirror circuit repeats should be unique for each circuit index."
                assert (
                    len(unique_circuit_labels) == 1
                ), "The circuit labels should be unique for each circuit index."

                dsd_i = unique_delay_schedules[0]
                mcr_i = unique_mcr[0]

                circuit_label = unique_circuit_labels[0]
                circuit_bitstrings = bck.array(
                    df_i[SNV.Bitstring.id_long].tolist(), dtype=np.int32
                )
                circuit_counts = bck.array(df_i[SNV.Count.id_long].tolist(), dtype=int)

                # raise KeyboardInterrupt

                df_res_here = pd.DataFrame(
                    data={
                        "DSDescription": [dsd_i] * len(circuit_counts),
                        "MCRepeats": [mcr_i] * len(circuit_counts),
                        SNV.CircuitLabel.id_long: [tuple(circuit_label)]
                        * len(circuit_counts),
    ...
    def calculate_noise_matrices(self, subsets_2q=None, include_2q=False):
        ...
        for delay in delays_list:
            for mcr in mirror_circuit_repeats_list:
                df_i = df_results[
                    (df_results["DSDescription"] == delay)
                    & (df_results["MCRepeats"] == mcr)
                ].copy()
                ...
                for input_state in all_input_states:
                    ...
                    bts_ij = bck.array(
                        df_i_j[SNV.Bitstring.id_long].tolist(), dtype=bck.uint8
                    )
                ...
                ddot_analyzer_i = DDOTAnalyzer(
                    circuits_labels=all_input_states,
                ...
                df_res_here = pd.DataFrame(
                    {
                        "DSDescription": [delay] * self._number_of_qubits,
                ...
                if include_2q:
                    noise_matrices_2q = list(
                        ddot_analyzer_i.get_2q_noise_matrices(
                            subsets_2q=subsets_2q, show_progress_bar=False
                        )
                    )
    ...
    def read_results(self, experiment_instances_ids=None, old_version=False):
        if old_version:
        ...
        df_results = self._results_logger.gather_results(
            data_type=SNDT.BitstringsHistograms,
            experiment_instance_ids=experiment_instances_ids,
        )
        df_results["CircuitLabel"] = df_results["CircuitLabel"].apply(
    ...
    def _read_results_depreciated(self):
        ...
        bitstrings_folder = self._results_logger.get_absolute_path_of_data_type(
            data_type=SNDT.BitstringsHistograms
        )
        metadata_folder = self._results_logger.get_absolute_path_of_data_type(
        ...
        for file_name_bts in files_bitstrings:
            ...
            table_name_main_bitstrings = self._results_logger._tnps.join(
                split_name_bts[0:-1]
            )

            for file_name_metadata in files_metadata:
                split_name_metadata = file_name_metadata.split(
                    self._results_logger._tnps
                )
                table_name_main_metadata = self._results_logger._tnps.join(
                    split_name_metadata[0:-1]
                )
            ...
            df_here_bts = self._results_logger.read_results(
                table_name=table_name_main_bitstrings,
                data_type=SNDT.BitstringsHistograms,
            ...
            if df_here_bts is None:
                continue
            df_here_metadata = self._results_logger.read_results(
                table_name=table_name_main_bitstrings,
                data_type=SNDT.CircuitsMetadata,
                filter_by_experiment_set=False,
                append_experiment_set_suffix=False,
    ...
    def plot_hamming_weight_histograms(
        self,
        df_results: Optional[pd.DataFrame] = None,
        fom_names_list: Tuple[str, ...] = ("HammingWeight", "HammingDistance"),
        ...
        len(delays_list) * len(mirror_circuit_repeats_list)

        number_of_qubits = len(input_states_to_plot[0])
        known_input_states = {
            tuple([0] * number_of_qubits): "|0...0>",
                ...
                for idx_delay, delay in enumerate(delays_list):
                    ...
                    subplot_titles = [
                        (
                            ...
                            if input_state in known_input_states
                            else f"Input state:|X>"
                        )
                    ]
                    ...
                    for idx_mcr, mcr in enumerate(mirror_circuit_repeats_list):
                        ...
                        bitstrings_array = bck.array(
                            df_i[SNV.Bitstring.id_long].tolist(), dtype=bck.uint8
                        )
                        bitstrings_counts = bck.array(
                            df_i[SNV.Count.id_long].tolist(), dtype=int
                        )
                        ...
                        if fom_name in ["hamming_weight", "HammingWeight"]:
                            ...
                            fom_values = np.sum(
                                bitstrings_array ^ input_state_array, axis=1
                            )
                        ...
                        fig.add_trace(
                            go.Histogram(
                                x=fom_values,
                                xbins=dict(start=0, end=number_of_qubits + 1, size=1),
                                marker=marker_dict,
                                opacity=1.0,
                                name=trace_name,
                                showlegend=True,
                    ...
                    state_name = (
                        known_input_states[input_state]
                        if input_state in known_input_states
                        else "|X>"
                    )
                    ...
                    fig.write_image(f"{file_name}.png", format="png", scale=3.0)
                    if save_html:
    ...
    def plot_average_noise_rates(
        ...
        df_1q = noise_matrices_df[
            noise_matrices_df["Subset"].apply(lambda x: len(x) == 1)
        ].copy()
        df_2q = noise_matrices_df[
            noise_matrices_df["Subset"].apply(lambda x: len(x) == 2)
        ].copy()
        ...
        df_1q["p(0->0)-p(1->1)"] = df_1q.apply(
            lambda row: row["p(0->0)"] - row["p(1->1)"], axis=1
        )

        df_1q.drop(columns=["NoiseMatrix", "Subset"], inplace=True)

        df_1q_av = anf.contract_dataframe_with_aggregating_functions(
            df=df_1q,
        ...
        df_2q_av = anf.contract_dataframe_with_aggregating_functions(
            df=df_2q,
            functions_to_apply=["mean", "median", "std", "min", "max"],
            grouping_columns=["DSDescription", "MCRepeats"],
        )
        ...
        for sub_name_list, fig_add in zip(
            [
                sub_names_1q,
                [
                    "p(11->11)",
            ...
            if fig_add == "1q":
                if plot_noise_bias:
                    ...
                    subplot_titles = [
                        f"1Q 'State Preservation Fidelity'<br>({additional_title_string})"
                    ]
            elif fig_add == "2q":
                subplot_titles = [
                    f"2Q 'State Preservation Fidelity'<br>({additional_title_string})"
                ]
            ...
            for idx_delay, delay in enumerate(delays_list):
                ...
                for sub_idx, sub_name in enumerate(sub_name_list):
                    if sub_name in ["p(1->1)", "p(0->0)", "p(0->0)-p(1->1)"]:
                        ...
                        df_1q_av_i[f"{sub_name}_min"].values
                        df_1q_av_i[f"{sub_name}_max"].values
                    ...
                    fig.add_trace(
                        go.Scatter(
                            ...
                            # add error bars
                            error_y=dict(
                                type="data", array=fom_i_std, visible=True, width=8
                            ),
                            mode="markers+lines",
                    ...
                    fig.update_xaxes(
                        title_text="Mirror Circuit Repetitions",
                        row=row_idx,
                        col=1,
                        showgrid=True,  # make sure they’re on
                        gridcolor=color_grid,
            ...
            if save_html:
                plotly.offline.plot(fig, filename=f"{folder_path}{file_name}.html")
            ...
            fig_png.update_layout(
                ...
                title=dict(
                    font=dict(size=40), y=0.98, yanchor="top"  # Move title up slightly
                ),
            )

            # Write PNG with explicit dimensions
            fig_png.write_image(
                file=f"{folder_path}{file_name}.png",
                format="png",
                width=1500,  # Explicit width in pixels
                height=1600,  # Explicit height in pixels
                scale=1.0,
            )  # Don't scale, use explicit dimensions
