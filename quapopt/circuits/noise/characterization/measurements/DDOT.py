...
class DDOTRunner(OverlappingTomographyRunnerBase):
        ...
        if logging_level is not None and logging_level != LoggingLevel.NONE:
            ...
            if results_logger_kwargs is None:
                raise NotImplementedError(
                    "If you want to log results, you need to provide results_logger_kwargs."
                )
    ...
    def generate_tomography_circuits_random(
        self,
        ...
        if include_standard_ddot_circuits:
            ...
            ), "If you want to include standard DDOT circuits, you need to generate at least 2 circuits."
        ...
        return self._generate_tomography_circuits_random(
...
class DDOTAnalyzer:
    def __init__(
        ...
        if results_logger_kwargs is None:
            assert (
                circuits_labels is not None and bitstrings_arrays is not None
        ...
        if circuits_labels is None:
            assert (
                bitstrings_arrays is None
            ), "If you provide bitstrings arrays, you need to provide circuit labels as well."
        if bitstrings_arrays is None:
            assert (
                circuits_labels is None
            ), "If you provide circuit labels, you need to provide bitstrings arrays as well."

        if self._results_logger is not None:
            anf.cool_print("READING DATA...", "...", "blue")
            results_df = self.read_results(
                table_name=None, data_type=SNDT.BitstringsHistograms, uuid=uuid
            )
            ...
            for circuit_index in tqdm(
                unique_circuit_indices, desc="PRE-PROCESSING", colour="yellow"
            ):
                results_df_job = results_df[
                    results_df[SNV.CircuitIndex.id_long] == circuit_index
                ]
                unique_circuit_labels = results_df_job[
                    SNV.CircuitLabel.id_long
                ].unique()
                if len(unique_circuit_labels) > 1:
                    raise ValueError(
                        f"More than one circuit label found for circuit id {circuit_index}."
                    )

                circuit_label = unique_circuit_labels[0]

                bitstrings_circuit = bck.array(
                    results_df_job[SNV.Bitstring.id_long].tolist(), dtype=bck.uint8
                )
                counts_circuit = bck.array(
                    results_df_job[SNV.Count.id_long].tolist(), dtype=int
                )

                bitstrings_circuit = bck.repeat(
                    bitstrings_circuit, counts_circuit, axis=0
                )
        ...
        assert len(circuits_labels) == len(
            bitstrings_arrays
        ), "Circuit labels and bitstrings arrays have different lengths."

        if noisy_sampler_post_processing is not None:
            bitstrings_arrays = [
                noisy_sampler_post_processing.add_noise_to_samples(
                    ideal_samples=bts_x, rng=noisy_sampler_post_processing_rng
                )
                for bts_x in bitstrings_arrays
            ]
        ...
        assert (
            len(self._circuits_labels[0]) == self._bitstrings_arrays.shape[2]
        ), "Circuit labels and bitstrings arrays have different lengths."
    ...
    def read_results(
        ...
        results_df = self._results_logger.read_results(
            table_name=table_name, data_type=data_type
        )
        if uuid is not None:
            assert (
                "UUID" in results_df.columns
            ), "UUID column not found in results dataframe."
            results_df = results_df[results_df["UUID"] == uuid].copy()

        if "CircuitLabel" in results_df.columns:
            results_df["CircuitLabel"] = results_df["CircuitLabel"].apply(
                anf.eval_string_tuple_to_tuple
            )
    ...
    def get_1q_noise_matrices(self):
        ...
        for i in range(2):
            noise_matrices[:, :, i] /= noise_matrices[:, :, i].sum(
    ...
    def get_2q_noise_matrices(self, subsets_2q=None, show_progress_bar=True):
        """
        Get the 2-qubit noise matrices from the bitstrings arrays.
        ...
        integer_idx_input = (circuits_labels << 1)[:, qi_idx] | circuits_labels[
            :, qj_idx
        ]
        number_of_subsets = len(subsets_2q)

        for circuit_index in tqdm(
            list(range(len(circuits_labels))),
            position=0,
            colour="green",
            ...
            for output_state_index in range(4):
                ...
                for id in range(number_of_subsets):
                    noise_matrices[
                        id, output_state_index, integer_idx_input[circuit_index, id]
                    ] += marg_i_2q[id]
    ...
    def _save_figure(self, figure, file_name):

        os.makedirs("/".join(file_name.split("/")[0:-1]), exist_ok=True)

        plotly.offline.plot(figure, filename=f"{file_name}.html")

    def plot_noise_matrices(
        self,
        noise_matrices=None,
        subset_indices=None,
        ...
        if noise_matrices is not None:
            ...
            ), "If you provide noise matrices, you need to provide subset indices as well."
        ...
        fig = make_subplots(
            ...
            subplot_titles=[
                f"Qubit(s):{physical_names[i]}" for i in range(len(noise_matrices))
            ],
        )
        ...
        self._save_figure(figure=fig, file_name=file_name)

    def plot_noise_matrices_1q(
        self,
        qubit_indices_physical_map: Optional[List[int]] = None,
        colormap_name="YlGnBu",
        file_name=None,
    ):
        self.plot_noise_matrices(
            noise_matrices=self.noise_matrices_1q,
            subset_indices=list(range(self._number_of_qubits)),
    ...
    def plot_noise_matrices_2q(
        ...
        self.plot_noise_matrices(
            noise_matrices=self.noise_matrices_2q,
    ...
    def plot_errors_histograms(self, noise_matrices=None, file_name=None):
        ...
        titles = [
            f"Prob({''.join([str(x) for x in cls[i]])}->{''.join([str(x) for x in cls[j]])})"
            for i in range(dimension)
            for j in range(dimension)
        ]

        fig = make_subplots(rows=dimension, cols=dimension, subplot_titles=titles)
        ...
        for input_int in range(dimension):
            for output_int in range(dimension):
                ...
                xbins_dict = dict(size=size_bin)  # start=bin_min,
                ...
                fig.add_trace(
                    go.Scatter(
                        ...
                        y=[
                            0,
                            max([np.histogram(probs_x_y, bins=xbins_arange)[0]]) * 1.0,
                        ],
                        mode="lines",
    ...
    def plot_errors_histograms_1q(self, file_name=None):
        self.plot_errors_histograms(
            noise_matrices=self.noise_matrices_1q, file_name=file_name
        )

    def plot_errors_histograms_2q(self, file_name=None):
        self.plot_errors_histograms(
            noise_matrices=self.noise_matrices_2q, file_name=file_name
        )
...
if __name__ == "__main__":
    ...
    CMNS = ClassicalMeasurementNoiseSampler(
        noise_type=MeasurementNoiseType.TP_1q_general,
        noise_description={"p_01": p01_list, "p_10": p10_list},
        rng=numpy_rng,
    )

    expected_noise_matrices_1q = [
        bck.array([[1 - p_10, p_01], [p_10, 1 - p_01]])
        for p_01, p_10 in zip(p01_list, p10_list)
    ]
    ...
    if number_of_qubits_test <= 24:
        ...
        random_circuits = DDOT_run_test.generate_tomography_circuits_random(
            number_of_circuits=number_of_circuits_test,
            enforce_uniqueness=True,
            add_measurements=True,
            add_barriers=True,
        ...
        qiskit_backend = provider.backend(name=backend_name, use_fractional_gates=True)
        pass_manager = generate_preset_pass_manager(
            backend=qiskit_backend, optimization_level=0
        )

        circuits_labels, bitstrings_histograms = (
            DDOT_run_test.run_tomography_circuits_qiskit(
        ...
        circuits_labels = rng_test.binomial(
            n=1, p=1 / 2, size=(number_of_circuits_test, number_of_qubits_test)
        ).astype(bck.uint8)
        bitstrings_array = bck.array(
            [[label] * number_of_samples_test for label in circuits_labels]
        )
        noisy_bitstrings_array = [
            CMNS.add_noise_to_samples(ideal_samples=row, rng=numpy_rng)
            for row in bitstrings_array
        ]
    ...
    subsets_2q = [
        ...
        for j in range(i + 1, number_of_qubits_test)
    ]

    expected_noise_matrices_2q = [
        bck.kron(expected_noise_matrices_1q[i], expected_noise_matrices_1q[j])
        for i, j in subsets_2q
    ]

    t0 = time.perf_counter()
    reconstructed_noise_matrices_1q = ddot_analyzer_test.get_1q_noise_matrices()
    t1 = time.perf_counter()
    reconstructed_noise_matrices_2q = ddot_analyzer_test.get_2q_noise_matrices(
        subsets_2q=subsets_2q
    )
    ...
    print(
        "1q matrices agree:",
        bck.allclose(
            bck.array(reconstructed_noise_matrices_1q),
    ...
    print(
        "2q matrices agree:",
        bck.allclose(
            bck.array(reconstructed_noise_matrices_2q),
            bck.array(expected_noise_matrices_2q),
            atol=1e-2,
        ),
    )

    ddot_analyzer_test.plot_noise_matrices(
        noise_matrices=reconstructed_noise_matrices_1q,
        subset_indices=list(range(number_of_qubits_test)),
    )
    #
    ddot_analyzer_test.plot_noise_matrices(
        noise_matrices=reconstructed_noise_matrices_2q, subset_indices=subsets_2q
    )

    ddot_analyzer_test.plot_errors_histograms(
        noise_matrices=reconstructed_noise_matrices_1q
    )
    ddot_analyzer_test.plot_errors_histograms(
        noise_matrices=reconstructed_noise_matrices_2q
    )

    raise KeyboardInterrupt()

    marginals_1q = em.get_marginals_from_bitstrings_array(
        bitstrings_array=random_bitstrings,
        qubits_subsets_by_locality={1: [0, 1, 3]},
        normalize=False,
    )
    marginals_2q = em.get_marginals_from_bitstrings_array(
        bitstrings_array=random_bitstrings,
        qubits_subsets_by_locality={2: [(0, 1), (1, 2), (2, 3)]},
        normalize=False,
    )
    marginals_3q_4q = em.get_marginals_from_bitstrings_array(
        bitstrings_array=random_bitstrings,
        qubits_subsets_by_locality={3: [(0, 1, 2), (1, 2, 3)], 4: [(0, 1, 2, 3)]},
        normalize=False,
    )
