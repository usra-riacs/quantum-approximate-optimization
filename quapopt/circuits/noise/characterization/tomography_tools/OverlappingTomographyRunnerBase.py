...
class OverlappingTomographyRunnerBase(QiskitSessionManagerMixin):
    ...
    def append_delays(self,
                      ...
                      duration: float,
                      duration_unit='us',
                      qubit_indices: Optional[List[int]] = None) -> CircuitQiskit:
        """
        ...
        if self._sdk_name in ['qiskit']:
            circuit.delay(duration=duration,
                          unit=duration_unit,
                          qarg=qubit_indices)
        else:
    ...
    def _generate_symbols_random(self,
                                 number_of_circuits: int,
                                 number_of_symbols: int,
                                 enforce_uniqueness=False,
                                 prepended_symbols_list=None) -> List[Tuple[int, ...]]:
        """
        ...
        if enforce_uniqueness:
            if self._number_of_qubits < 20:
                ...
                if number_of_circuits + len(prepended_symbols_list) > possible_combinations:
                    ...
                    raise ValueError("Cannot enforce uniqnuess for more circuits than possible combinations")
        ...
        if number_of_symbols == 1:
            ...
            random_symbols = self._numpy_rng.binomial(n=1, p=1 / 2,
                                                      size=(number_of_circuits, self._number_of_qubits))

        else:
            random_symbols = self._numpy_rng.integers(low=0,
        ...
        random_symbols = prepended_symbols_list + [tuple(x.tolist()) for x in random_symbols]
        if enforce_uniqueness:
            random_symbols = list(set(random_symbols))
            if len(random_symbols) < number_of_circuits + len(prepended_symbols_list):
                missing_symbols = number_of_circuits + len(prepended_symbols_list) - len(random_symbols)
                return self._generate_symbols_random(number_of_circuits=missing_symbols,
    ...
    def _generate_symbols_all_combinations(self,
                                           number_of_symbols: int) -> List[Tuple[int, ...]]:
        """
        ...
        return [tuple(x) for x in itertools.product(range(number_of_symbols), repeat=self._number_of_qubits)]

    def _get_tomography_circuit_nonparametric_pauli(self,
                                                    circuit_label: Union[
                                                    ...
                                                    add_barriers=True) -> AbstractCircuit:
        """
        ...
        if middle_circuit_global is not None:
            assert self._tomography_type == TomographyType.PROCESS, "Middle circuit is only supported for process tomography"
        ...
        elif self._tomography_type in [TomographyType.DIAGONAL_DETECTOR,
        ...
        if self._sdk_name in ['qiskit']:
            ...
            circuit = QuantumCircuit(self._number_of_qubits_device_qiskit,
        ...
        if prepend_circuit_global is not None:
            circuit = self._program_gate_builder.combine_circuits(left_circuit=circuit,
                                                                  right_circuit=prepend_circuit_global)
        ...
        if circuit_symbols_prepend is not None:
            ...
            for symbol_i in range(number_of_symbols):
                qi_physical = [_qubit_indices[index_i] for index_i, label_i in
                               enumerate(circuit_symbols_prepend) if label_i == symbol_i]
                if symbol_i == 0:
                    # Initialize in |0>
                    circuit = self._program_gate_builder.I(quantum_circuit=circuit,
                                                           qubits_tuple=qi_physical)
                elif symbol_i == 1:
                    # Initialize in |1>
                    circuit = self._program_gate_builder.X(quantum_circuit=circuit,
                                                           qubits_tuple=qi_physical)
                elif symbol_i == 2:
                    # Initialize in |+>
                    circuit = self._program_gate_builder.H(quantum_circuit=circuit,
                                                           qubits_tuple=qi_physical)
                elif symbol_i == 3:
                    # Initialize in |->
                    circuit = self._program_gate_builder.X(quantum_circuit=circuit,
                    ...
                    circuit = self._program_gate_builder.H(quantum_circuit=circuit,
                                                           qubits_tuple=qi_physical)
                    circuit = self._program_gate_builder.S(quantum_circuit=circuit,
                                                           qubits_tuple=qi_physical)
                elif symbol_i == 5:
                    # Initialize in |i->
                    circuit = self._program_gate_builder.X(quantum_circuit=circuit,
                    ...
                    raise ValueError(f'Pauli detector tomography_tools supports only 6 labels, got {symbol_i}')

        if middle_circuit_global is not None:
            circuit = self._program_gate_builder.combine_circuits(left_circuit=circuit,
                                                                  right_circuit=middle_circuit_global)
        ...
        if circuit_symbols_append is not None:
            ...
            for symbol_i in range(number_of_symbols):
                qi_physical = [_qubit_indices[index_i] for index_i, label_i in
                               enumerate(circuit_symbols_append) if label_i == symbol_i]
                if symbol_i == 0:
                    ...
                    # Measure in Y basis
                    circuit = self._program_gate_builder.Sdag(quantum_circuit=circuit,
                                                              qubits_tuple=qi_physical)
                    circuit = self._program_gate_builder.H(quantum_circuit=circuit,
                                                           qubits_tuple=qi_physical)
                else:
                    raise ValueError(f'Pauli detector tomography_tools supports only 3 labels, got {symbol_i}')

        if append_circuit_global is not None:
            # print(len(bck_utils.get_nontrivial_physical_indices_from_circuit(quantum_circuit=circuit)), len(circuit.qubits))
            ...
            # print(len(circuit.qubits))
            # print(len(append_circuit_global.qubits))
            # print(circuit)
            # print('lol')
            # print(append_circuit_global)
            circuit = self._program_gate_builder.combine_circuits(left_circuit=circuit,
                                                                  right_circuit=append_circuit_global)

        if add_barriers:
            if self._sdk_name != 'qiskit':
                raise NotImplementedError(f"Barriers are not supported in {self._sdk_name} yet.")
            circuit = self.append_barriers(circuit=circuit,
                                           qubit_indices=_qubit_indices)

        if add_measurements:
            circuit = self.append_measurements(circuit=circuit,
                                               qubit_indices=_qubit_indices)
        return circuit

    def _generate_tomography_circuits_nonparametric_random(self,
                                                           ...
                                                           prepended_symbols_list: Optional[
                                                               List[Tuple[int, ...]]] = None,
                                                           ) -> List[
        ...
        if prepended_symbols_list is not None:
            assert len(prepended_symbols_list) <= number_of_circuits, (f"If you want to prepend symbols, "
        ...
        if self._tomography_type == TomographyType.PROCESS:
            ...
            circuit_symbols_1 = self._generate_symbols_random(number_of_circuits=number_of_circuits,
                                                              number_of_symbols=self._number_of_symbols[0],
                                                              prepended_symbols_list=prepended_symbols_list)
            circuit_symbols_2 = self._generate_symbols_random(number_of_circuits=number_of_circuits,
                                                              number_of_symbols=self._number_of_symbols[1],
                                                              prepended_symbols_list=prepended_symbols_list)
            circuit_symbols = list(zip(circuit_symbols_1, circuit_symbols_2))
        else:
            circuit_symbols = self._generate_symbols_random(number_of_circuits=number_of_circuits,
                                                            number_of_symbols=self._number_of_symbols,
                                                            enforce_uniqueness=enforce_uniqueness,
                                                            prepended_symbols_list=prepended_symbols_list)
        ...
        for circuit_label in circuit_symbols:
            circuit_i = self._get_tomography_circuit_nonparametric_pauli(circuit_label=circuit_label,
    ...
    def _generate_tomography_circuits_random(self,
                                             number_of_circuits=1,
                                             enforce_uniqueness=True,
                                             add_measurements=True,
                                             add_barriers=True,
        ...
        if self._parametric:
            raise NotImplementedError("Parametric tomography_tools is not supported yet.")
        else:
            return self._generate_tomography_circuits_nonparametric_random(number_of_circuits=number_of_circuits,
                                                                           enforce_uniqueness=enforce_uniqueness,
                                                                           add_measurements=add_measurements,
                                                                           add_barriers=add_barriers,
...
if __name__ == '__main__':
    ...
    OTR_test = OverlappingTomographyRunnerBase(number_of_qubits=noq_test,
    ...
    random_circuits = OTR_test._generate_tomography_circuits_random(number_of_circuits=2 ** noq_test,
                                                                    enforce_uniqueness=True,
    ...
    for delay in delays_list:
        ...
        for circ_label, circ in circuits_delay:
            circ = OTR_test.append_delays(circuit=circ,
                                          duration=delay,
                                          duration_unit='s')
            circ = OTR_test.append_measurements(circuit=circ)
            circuits_delay_run.append((circ_label, circ))

        df_res_delay = OTR_test.run_tomography_circuits_qiskit(tomography_circuits=circuits_delay_run,
                                                               qiskit_backend=qiskit_backend,
                                                               shots=1000,
                                                               df_annotation={'delay': delay,
                                                                              'delay_unit': 's'})
