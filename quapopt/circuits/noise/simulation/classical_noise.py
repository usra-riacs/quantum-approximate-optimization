...
def get_stochastic_matrix_1q(
    ...
    return np.array([[1 - p_10, p_01], [p_10, 1 - p_01]])


def create_tensor_product_matrix(
    matrices_dictionary: Dict[Tuple[int], np.ndarray],
) -> np.ndarray:
    """
    ...
    for node_ids, local_matrix in matrices_dictionary.items():
        embedded_local_matrix = em.embed_operator_in_bigger_hilbert_space(
...
def add_tensor_product_stochastic_noise_to_samples(
    ideal_samples_list: Union[List[Tuple[int, ...]], np.ndarray],
    ...
    ideal_samples = pd.DataFrame(
        data={
            "experiment_id": ideal_samples_list,
    ...
    classical_registers_local = {
        size: anf.get_full_classical_register(
            number_of_bits=size, register_format=tuple
        )
    ...
    marginals_df = marg_fun.get_marginals_from_bitstrings(
        results_dataframe=ideal_samples,
    ...
    marginals_df["state_int_count"] = marginals_df.apply(
        lambda row: row["marginal"][row["state_int"]][0], axis=1
    )

    time.perf_counter()

    all_values_sets_exp = {
        exp_id: {"qi": [], "si": []} for exp_id in ideal_samples_list
    }
    for subset, noise_matrix in tqdm(
        list(stochastic_matrices_dictionary.items()), disable=not show_progress_bar
    ):
        local_register = classical_registers_local[len(subset)]
        marginals_subset = anf.find_dataframe_subset(
            df=marginals_df, variable_values_pairs=[("subset", subset)]
        )
        marginals_subset.drop(columns=["marginal"], inplace=True)
        marginals_subset_ints_counted = anf.contract_dataframe_with_functions(
            df=marginals_subset,
        ...
        for state_int, state_int_count, exp_id in zip(
            marginals_subset_ints_counted["state_int"].values,
            ...
            if state_int_count == 0:
                continue
            random_samples_int = rng.multinomial(
                n=state_int_count, pvals=noise_matrix[:, state_int]
            )
            sequence = []
    ...
    for key, samples_separated in all_values_sets_exp.items():
        ...
        for ind_enum, (q_inds, s_values) in enumerate(
            zip(qubit_indices, sample_indices)
        ):
...
def _add_identical_1q_tensor_product_noise_to_samples(
    ideal_samples_array: Union[np.ndarray, cp.ndarray],
    p_01: float = None,
    p_10: float = None,
    rng=None,
    ...
    if p_01 != 0.0:
        ...
        ideal_samples_array[ones_mask] = ideal_samples_array[ones_mask] ^ rng.binomial(
            n=1, p=p_01, size=ones_size
        ).astype(bck.uint8)

    if p_10 != 0.0:
        zeros_size = int(zeros_mask.sum())
        ideal_samples_array[zeros_mask] = ideal_samples_array[
            zeros_mask
        ] ^ rng.binomial(n=1, p=p_10, size=zeros_size).astype(bck.uint8)

    return ideal_samples_array


def _add_nonidentical_1q_tensor_product_noise_to_samples(
    ideal_samples_array: Union[np.ndarray, cp.ndarray],
    ...
    if p_01_list is not None:
        ...
        bits_flipped_or_not_ones = bck.array(
            [
                rng.binomial(n=1, p=p_01, size=number_of_samples)
                for qi, p_01 in enumerate(p_01_list)
            ],
            dtype=bck.uint8,
        ).T

        ideal_samples_array = ideal_samples_array ^ (
            bits_flipped_or_not_ones * ones_mask
        )
    ...
    if p_10_list is not None:
        ...
        bits_flipped_or_not_zeros = bck.array(
            [
                rng.binomial(n=1, p=p_10, size=number_of_samples)
                for qi, p_10 in enumerate(p_10_list)
            ],
            dtype=bck.uint8,
        ).T
        ideal_samples_array = ideal_samples_array ^ (
            bits_flipped_or_not_zeros * zeros_mask
        )

    return ideal_samples_array


def add_1q_tensor_product_noise_to_samples(
    ideal_samples_array: Union[np.ndarray, cp.ndarray],
    ...
    if isinstance(p_01_errors, float) and isinstance(p_10_errors, float):
        return _add_identical_1q_tensor_product_noise_to_samples(
            ideal_samples_array=ideal_samples_array,
            p_01=p_01_errors,
            p_10=p_10_errors,
            rng=rng,
        ...
        return _add_nonidentical_1q_tensor_product_noise_to_samples(
            ideal_samples_array=ideal_samples_array,
...
if __name__ == "__main__":
    ...
    for i in range(number_of_qubits_test):
        ...
        stochastic_matrix_test = get_stochastic_matrix_1q(
            p_10=p_10_test, p_01=p_01_test
        )
        noise_matrices_dict[(i,)] = stochastic_matrix_test

    global_noise_matrix = create_tensor_product_matrix(
        matrices_dictionary=noise_matrices_dict
    )

    full_register_test = anf.get_full_classical_register(
        number_of_bits=number_of_qubits_test, register_format=tuple
    )
    ideal_samples_test = []
    ideal_distro_test = []
    ...
    for bts_test in full_register_test:
        number_of_samples = numpy_rng_test.integers(
            low=0, high=int(total_number_of_samples_max / dim_test)
        )
    ...
    noisy_samples_test = add_tensor_product_stochastic_noise_to_samples(
        ideal_samples_list=ideal_samples_test,
    ...
    anf.cool_print(
        "Conservative sample error bound (samples):",
