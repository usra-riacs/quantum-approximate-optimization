...
from quapopt.circuits.noise.simulation.classical_noise import (
    add_1q_tensor_product_noise_to_samples,
)
...
from quapopt.hamiltonians.representation.ClassicalHamiltonianBase import (
    get_fields_and_couplings_from_hamiltonian_list,
)
...
class ClassicalMeasurementNoiseSampler:
    def __init__(
        ...
        if self.noise_type in [
            MeasurementNoiseType.TP_1q_identical,
            MeasurementNoiseType.TP_1q_general,
        ]:

            if "p_01" not in noise_description:
                raise ValueError(
                    "p_01 must be specified for TensorProduct1QIdentical noise"
                )
            if "p_10" not in noise_description:
                raise ValueError(
                    "p_10 must be specified for TensorProduct1QIdentical noise"
                )
            ...
            if p01_list_or_float is not None:
                if self.noise_type == MeasurementNoiseType.TP_1q_identical:
                    assert isinstance(
                        p01_list_or_float, float
                    ), "p_01 must be a float for TensorProduct1QIdentical noise"
                elif self.noise_type == MeasurementNoiseType.TP_1q_general:
                    assert not isinstance(
                        p01_list_or_float, float
                    ), "p_01 must be a list or array for TensorProduct1QGeneral noise"
            ...
            if p10_list_or_float is not None:
                if self.noise_type == MeasurementNoiseType.TP_1q_identical:
                    assert isinstance(
                        p10_list_or_float, float
                    ), "p_10 must be a float for TensorProduct1QIdentical noise"
                elif self.noise_type == MeasurementNoiseType.TP_1q_general:
                    assert not isinstance(
                        p10_list_or_float, float
                    ), "p_10 must be a list or array for TensorProduct1QGeneral noise"
            ...
            def _sampling_function(
                ideal_samples: Union[np.ndarray, cp.ndarray], rng=self._rng
            ) -> np.ndarray:

                return add_1q_tensor_product_noise_to_samples(
                    ideal_samples_array=ideal_samples,
            ...
            self._noise_matrices_1q = self._get_1q_noise_matrices(
                p_01_errors=p01_list_or_float, p_10_errors=p10_list_or_float
            )

            noise_description = {"p_01": p01_list_or_float, "p_10": p10_list_or_float}
    ...
    def add_noise_to_samples(
        self, ideal_samples: Union[cp.ndarray, np.ndarray], rng=None
    ):
        if rng is None:
            rng = self._rng
        ...
        return self._sampling_function(ideal_samples=ideal_samples.copy(), rng=rng)

    def _get_1q_noise_matrices(
        self,
        ...
        if self.noise_type == MeasurementNoiseType.TP_1q_identical:
            ...
            assert isinstance(p_10_errors, float) and isinstance(
                p_01_errors, float
            ), "p_01 and p_10 must be floats for TensorProduct1QIdentical noise"

            lam_1q = bck.array(
                [[1 - p_10_errors, p_01_errors], [p_10_errors, 1 - p_01_errors]]
            )
            return lam_1q
        elif self.noise_type == MeasurementNoiseType.TP_1q_general:
            ...
            return [
                ...
                for p_01, p_10 in zip(p_01_errors, p_10_errors)
            ]

    def get_full_noise_matrix_representation(self, number_of_qubits, transpose=False):
        ...
        if self.noise_type == MeasurementNoiseType.TP_1q_identical:
            ...
            full_matrix = 1.0
            for _ in tqdm(
                list(range((number_of_qubits))), disable=number_of_qubits < 10
            ):
    ...
    def get_noise_matrix_element(
        ...
        if self.noise_type == MeasurementNoiseType.TP_1q_identical:
            ...
            p_xy = 1.0
            for qi_index, qi_input, qi_output in enumerate(
                zip(input_state, output_state)
            ):
            ...
            raise NotImplementedError(
                "Only implemented for TensorProduct1QIdentical noise"
            )

    def get_probability_of_particular_output_state(
        self,
        output_state,
        ...
        if self.noise_type == MeasurementNoiseType.TP_1q_identical:
            assert (
                number_of_qubits is not None
            ), "number_of_qubits must be specified for TensorProduct1QIdentical noise"

            self._noise_matrices_1q
            ...
            if classical_register is None:
                classical_register = em.cuda_generate_classical_register(
                    number_of_qubits
                )
    ...
    def transform_hamiltonian_to_noisy_version(
        self, hamiltonian_list: List[Tuple[Union[float, int], Tuple[int, ...]]]
    ) -> Tuple[List[Tuple[Union[float, int], Tuple[int, ...]]], Union[float, int]]:
        ...
        correlations_matrix = np.zeros(
            (number_of_qubits, number_of_qubits), dtype=float
        )
        ...
        if self.noise_type == MeasurementNoiseType.TP_1q_identical:
            ...
            if (
                self.noise_description["p_10"] != 0
                and self.noise_description["p_10"] is not None
            ):
                raise NotImplementedError("Only implemented for p_10 = 0")
            ...
            identity_factor += (p_01**2) * np.sum(
                np.sum(correlations_matrix / 2, axis=0)
            )
            ...
            hamiltonian_noisy = [
                (ci, (qi,))
                for qi, ci in enumerate(single_qubit_terms_noisy)
                if ci != 0.0
            ]
            for coeff, tup in hamiltonian_list:
                if len(tup) == 2:
    ...
    def add_noisy_hamiltonian_representations(
        self, hamiltonian_representations_dict: dict, hamiltonian_identifier="cost"
    ):
        ...
        if self._noisy_hamiltonian_fields[hamiltonian_identifier] is None:
            self._noisy_hamiltonian_fields[hamiltonian_identifier] = {
                key: {} for key in hamiltonian_representations_dict
            }
        if self._noisy_hamiltonian_couplings[hamiltonian_identifier] is None:
            self._noisy_hamiltonian_couplings[hamiltonian_identifier] = {
                key: {} for key in hamiltonian_representations_dict
            }

        for key, hamiltonian_representation in hamiltonian_representations_dict.items():
            hamiltonian_noisy, noisy_constant = (
                self.transform_hamiltonian_to_noisy_version(hamiltonian_representation)
            )

            fields_cost, correlations_cost = (
                get_fields_and_couplings_from_hamiltonian_list(
                    hamiltonian=hamiltonian_noisy,
            ...
            self._noisy_hamiltonian_representations[hamiltonian_identifier][key] = (
                hamiltonian_noisy,
                noisy_constant,
            )
            self._noisy_hamiltonian_fields[hamiltonian_identifier][key] = {
                "numpy": fields_cost,
                "cuda": fields_cost_cuda,
            }
            self._noisy_hamiltonian_couplings[hamiltonian_identifier][key] = {
                "numpy": correlations_cost,
                "cuda": correlations_cost_cuda,
            }

    def solve_noisy_hamiltonians(self, number_of_qubits, hamiltonian_identifier="cost"):
        ...
        for key, (
            ...
            noisy_constant,
        ) in self._noisy_hamiltonian_representations[hamiltonian_identifier].items():
            simulator_key = simulator(
                number_of_qubits, terms=hamiltonian_representation
            )
            self._noisy_hamiltonian_spectra[hamiltonian_identifier][
                key
            ] = simulator_key.get_cost_diagonal()
...
if __name__ == "__main__":
    ...
    CMNS = ClassicalMeasurementNoiseSampler(
        noise_type=MeasurementNoiseType.TP_1q_identical,
        noise_description={"p_01": p_01, "p_10": None},
    )
    ...
    p_noisy_one = CMNS.get_probability_of_particular_output_state(
        output_state=tuple([0] * noq_test),
        input_probability_distribution=prob_distro_one,
        number_of_qubits=noq_test,
    )

    p_noisy_zero = CMNS.get_probability_of_particular_output_state(
        output_state=tuple([0] * noq_test),
        input_probability_distribution=prob_distro_zero,
        number_of_qubits=noq_test,
    )

    p_noisy_plus = CMNS.get_probability_of_particular_output_state(
        output_state=tuple([0] * noq_test),
