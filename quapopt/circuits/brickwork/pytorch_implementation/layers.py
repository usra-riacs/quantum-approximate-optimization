...
from quapopt.circuits.brickwork.pytorch_implementation.gates import (
    get_1q_unitary,
    get_2q_unitary,
...
def apply_1q_layer(
    ...
    device: torch.device,
    precomputed_gates: Optional[dict] = None,
) -> torch.Tensor:
    """
    ...
    if isinstance(gate_descriptors, GateDescriptor):
        ...
        if len(gate_descriptors_list) != number_of_qubits:
            raise ValueError(
                f"Number of gate descriptors ({len(gate_descriptors_list)}) "
    ...
    for qubit_index in range(number_of_qubits):
        ...
        if gate_descriptor.diagonal:
            ...
            quantum_state = torch.tensordot(
                gate_1q_i, quantum_state, dims=([1], [qubit_index])
            )
...
def apply_2q_layer(
    ...
    for ord_index, qubit_index in enumerate(range(start_index, number_of_qubits, 2)):
        ...
        if gate_descriptor.diagonal:
            ...
            # Standard tensor contraction for non-diagonal gates
            quantum_state = torch.tensordot(
                gate_2q_i.view(_view_2q), quantum_state, dims=([2, 3], [qi, qj])
            )
...
def apply_brickwork_ansatz(
    ...
    precomputed_gates: Optional[Dict[str, torch.Tensor]] = None,
) -> torch.Tensor:
    """
    ...
    if quantum_state is None:
        ...
        quantum_state = torch.zeros(
            [2] * number_of_qubits, dtype=torch.complex64, device=device
        )
    ...
    even_gates_count, odd_gates_count = get_brickwork_2q_gate_count(
        number_of_qubits=number_of_qubits
    )

    for layer_index in range(number_of_layers):
        # 1-qubit gates layer
        angles_1q_i = angles_1q[
            layer_index * angles_per_layer_1q : (layer_index + 1) * angles_per_layer_1q
        ]
