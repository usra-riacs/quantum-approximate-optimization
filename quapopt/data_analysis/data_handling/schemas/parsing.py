...
from quapopt.data_analysis.data_handling.schemas.naming import (
    ...
    HamiltonianClassSpecifierMAXkSAT, HamiltonianClassSpecifierWishartPlantedEnsemble,
...
def reconstruct_hamiltonian_instance_specifier(instance_description: str, class_specifier):
    """
    ...
    _SN = STANDARD_NAMES_VARIABLES
    _SNH = STANDARD_NAMES_HAMILTONIAN_DESCRIPTIONS
    number_of_qubits = instance_data.get(_SN.NumberOfQubits.id_long, instance_data.get(_SN.NumberOfQubits.id))
    hamiltonian_instance_index = instance_data.get(_SNH.HamiltonianInstanceIndex.id_long,
                                                   instance_data.get(_SNH.HamiltonianInstanceIndex.id))

    # Check if we need EdgeProbabilityOrAmount
    edge_probability_or_amount = instance_data.get(_SNH.EdgeProbabilityOrAmount.id_long,
                                                   instance_data.get(_SNH.EdgeProbabilityOrAmount.id))

    # Check if we need WishartDensity
    wishart_density = instance_data.get(_SNH.WishartDensity.id_long, instance_data.get(_SNH.WishartDensity.id))
