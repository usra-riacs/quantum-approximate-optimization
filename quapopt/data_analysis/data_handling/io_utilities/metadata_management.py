...
from quapopt.data_analysis.data_handling.schemas import BaseNameDataType
...
class ExperimentSetMetadataConfig:
    ...
    def __post_init__(self):
        if self.ExperimentSetName is None and self.ExperimentSetId is None:
            raise ValueError(
                "At least one of the: ExperimentSetName and ExperimentSetId should be provided"
            )
    ...
    def from_dataframe(cls, dataframe: pd.DataFrame):
        ...
        df = dataframe.iloc[0]
        return cls(
            ExperimentSetName=df[SNV.ExperimentSetName.id_long],
            ExperimentSetId=df[SNV.ExperimentSetID.id_long],
            directory_main=(
                df["main_directory"] if pd.notna(df["main_directory"]) else None
            ),
            default_storage_directory=(
                df["default_storage_directory"]
                if pd.notna(df["default_storage_directory"])
                else Path(DEFAULT_STORAGE_DIRECTORY)
            ),
            description=df["description"] if pd.notna(df["description"]) else None,
            created_timestamp=(
                df["created_timestamp"]
                if pd.notna(df["created_timestamp"])
                else anf.get_current_date_time()
            ),
        )
...
class ExperimentSetMetadataManager(IOMixin):
    ...
    def __init__(
        ...
        config = ExperimentSetMetadataConfig(
            ExperimentSetName=experiment_set_name,
    ...
    @staticmethod
    def _convert_to_set_level_metadata(
    ...
    @classmethod
    def from_config(
        cls, config: ExperimentSetMetadataConfig
        ...
        """
        return cls(
            experiment_set_name=config.ExperimentSetName,
    ...
    def construct_base_path(self):
        return super().construct_base_path(
            directory_main=self.main_directory,
    ...
    def from_name_and_id(
        ...
        if existing_config_df is not None:
            read_exp_names_list = existing_config_df[
                SNV.ExperimentSetName.id_long
            ].tolist()
            ...
            manager = cls.from_config(
                config=ExperimentSetMetadataConfig.from_dataframe(
                    dataframe=existing_config_df
                )
            )
            ...
            if instance_ids:  # Only add if there are actual instance IDs
                manager.add_experiment_instances(
                    experiment_instance_ids=instance_ids, write_to_tracking=False
                )
            return manager
    ...
    def add_experiment_instances(
        self, experiment_instance_ids: List[str], write_to_tracking: bool = True
    ) -> Optional[Path]:
    ...
    def get_experiment_set_suffix(self) -> str:
        return self.get_key_value_pair(
            key_id=SNV.ExperimentSetID.id, value=self.experiment_set_id
        )
    ...
    def get_metadata_filename(
        ...
        experiment_set_suffix = cls.get_key_value_pair(
            key_id=SNV.ExperimentSetID.id, value=experiment_set_id
        )

        table_name_parts = [table_name_prefix, table_name_suffix, experiment_set_suffix]

        return cls.get_full_table_name(
            table_name_parts=table_name_parts, data_type=metadata_data_type
        )

    def get_metadata_path(self, metadata_data_type: BaseNameDataType) -> Path:
    ...
    def read_shared_metadata(
        ...
        if not isinstance(persistence_strategy, SplitFilePersistence):
            raise ValueError(
                "load_shared_metadata only works with SplitFilePersistence strategy"
            )

        return persistence_strategy.read_set_metadata(
            self, metadata_data_type, table_name_prefix, table_name_suffix
        )

    def write_shared_metadata(
        self,
        metadata_data_type: BaseNameDataType,
        shared_metadata: Any,
        ...
        if not isinstance(persistence_strategy, SplitFilePersistence):
            raise ValueError(
                "save_shared_metadata only works with SplitFilePersistence strategy"
            )
...
class MetadataPersistenceStrategy(ABC):
    ...
    @staticmethod
    @abstractmethod
    def write_set_tracking(metadata_manager: ExperimentSetMetadataManager) -> Path:
        pass
...
class SplitFilePersistence(MetadataPersistenceStrategy):
    ...
    def _get_json_file_path(  # self,
        ...
        metadata_json_path = add_file_format_suffix(
            string=metadata_json_path, suffix=".json"
        )
    ...
    def write_set_tracking(  # cls,
        ...
        config_csv_path = SplitFilePersistence._get_csv_file_path(
        ...
        existing_df = SplitFilePersistence.read_set_tracking(
            metadata_manager=metadata_manager, return_none_if_not_found=True
        )
        ...
        if existing_df is not None:
            existing_instance_ids = set(
                existing_df[SNV.ExperimentInstanceID.id_long].tolist()
            )
            if (
                set(metadata_manager.experiment_instances_ids).difference(
    ...
    def read_set_tracking(  # cls,
        ...
        if not config_csv_path.exists():
            if return_none_if_not_found:
                ...
                raise FileNotFoundError(
                    f"Metadata tracking file not found: {config_csv_path}"
                )
        ...
        df[SNV.ExperimentSetName.id_long] = df[SNV.ExperimentSetName.id_long].astype(
            str
        )
        df[SNV.ExperimentInstanceID.id_long] = df[
            SNV.ExperimentInstanceID.id_long
        ].astype(str)

        df = df[df[SNV.ExperimentSetID.id_long] == metadata_manager.experiment_set_id]
        df = df[
            df[SNV.ExperimentSetName.id_long] == metadata_manager.experiment_set_name
        ]
    ...
    def write_set_metadata(  # cls,
        ...
        metadata_json_path = SplitFilePersistence._get_json_file_path(
            metadata_manager=metadata_manager,
            metadata_data_type=metadata_data_type,
            table_name_prefix=table_name_prefix,
            table_name_suffix=table_name_suffix,
    ...
    def read_set_metadata(  # cls,
        ...
        if not metadata_json_path.exists():
            raise FileNotFoundError(
                f"Shared metadata file not found: {metadata_json_path}"
            )
        ...
        if isinstance(metadata, dict) and metadata.get("_type") == "pandas_dataframe":
            return pd.DataFrame(metadata["data"], columns=metadata["columns"])
